...
bool CEditor::CallbackOpenMap(const char *pFilename, int StorageType, void *pUser)
{
	...
	if(MultiMapping.m_State >= CMultiMappingSession::STATE_CONNECTING)
	{
		...
		// Owner: loading a different map ends the shared session — warn and
...
void CEditor::RenderLayers(CUIRect LayersBox)
{
	...
	if(State.m_Operation == ELayerOperation::NONE)
	{
		if(State.m_PreviousOperation == ELayerOperation::GROUP_DRAG)
		{
			...
			m_MultiMappingSession.NotifyGroupProp(State.m_InitialGroupIndex, (int)EGroupProp::ORDER, Map()->m_SelectedGroup);
		}
		else if(State.m_PreviousOperation == ELayerOperation::LAYER_DRAG)
		{
			if(State.m_InitialGroupIndex != Map()->m_SelectedGroup)
			{
				...
				for(int i = (int)State.m_vInitialLayerIndices.size() - 1; i >= 0; --i)
					m_MultiMappingSession.NotifyDelLayer(State.m_InitialGroupIndex, State.m_vInitialLayerIndices[i]);
			...
			else
			{
				...
				for(int k = 0; k < (int)vLayerIndices.size(); k++)
				{
					...
					// RecordAction doesn't call Redo(), so mirror its MultiMapping sync here.
					m_MultiMappingSession.NotifyLayerProp(Map()->m_SelectedGroup, State.m_vInitialLayerIndices[k], (int)ELayerProp::ORDER, LayerIndex);
				}
...
void CEditor::RenderMenubar(CUIRect MenuBar)
{
	...
	// MultiMapping status log in menubar — show most recent entry
	...
	// MultiMapping ping indicator — always visible while STATE_LIVE
...
void CEditor::Render()
{
	...
	m_Prompt.Render();
	m_FontTyper.Render();
	m_MultiMappingSession.OnRender(View);
