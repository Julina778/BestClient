	...
	float Intra)
{
	...
	// Upstream screen cull, but allow TClient offscreen ghost-swap path
	if(!ScreenRect.Inside(Position) && !(g_Config.m_TcShowOthersGhosts && g_Config.m_TcSwapGhosts && !Local && ClientId >= 0))
		return;
	if(!GameClient()->OptimizerAllowRenderPos(Position))
		return;
	bool OtherTeam = GameClient()->IsOtherTeam(ClientId);
