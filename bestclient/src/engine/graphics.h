...
public:
	...
	enum class EPostProcessShader
	{
		WORLD_EDITOR_COPY = 0,
		WORLD_EDITOR_MOTION_BLUR,
		WORLD_EDITOR_BLOOM_PREFILTER,
		WORLD_EDITOR_BLOOM_BLUR_H,
		WORLD_EDITOR_BLOOM_BLUR_V,
		WORLD_EDITOR_BLOOM_COMPOSITE,
		WORLD_EDITOR_OUTLINE,
		WORLD_EDITOR_COMBINED,
	};

	struct CPostProcessParams
	{
		float m_aValues[8] = {};
	};
	...
	virtual bool UpdateTextureRaw(CTextureHandle TextureId, int x, int y, size_t Width, size_t Height, const CImageInfo &Image, bool IsMovedPointer) = 0;
	virtual CTextureHandle LoadTexture(const char *pFilename, int StorageType, int Flags = 0) = 0;
	virtual void TextureSet(CTextureHandle Texture) = 0;
	void TextureClear() { TextureSet(CTextureHandle()); }
	virtual CRenderTargetHandle CreateRenderTarget(uint32_t Width, uint32_t Height) = 0;
	virtual void DeleteRenderTarget(CRenderTargetHandle *pRenderTarget) = 0;
	virtual CTextureHandle GetRenderTargetTexture(CRenderTargetHandle RenderTarget) = 0;
	virtual void CaptureScreenToRenderTarget(CRenderTargetHandle RenderTarget) = 0;
	virtual void BeginRenderTarget(CRenderTargetHandle RenderTarget) = 0;
	virtual void EndRenderTarget() = 0;
	virtual void RenderPostProcess(CRenderTargetHandle DstRenderTarget, EPostProcessShader Shader, CTextureHandle SrcTexture0, CTextureHandle SrcTexture1, const CPostProcessParams &Params) = 0;
