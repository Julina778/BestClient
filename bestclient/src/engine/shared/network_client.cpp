...
bool CNeBestClient::Open(NETADDR BindAddr)
{
	// open socket
	NETSOCKET Socket;
...
void CNeBestClient::Disconnect(const char *pReason)
{
	m_Connection.Disconnect(pReason);
}

void CNeBestClient::Update()
{
	m_Connection.Update();
...
void CNeBestClient::Connect(const NETADDR *pAddr, int NumAddrs)
{
	m_Connection.Connect(pAddr, NumAddrs);
}

void CNeBestClient::Connect7(const NETADDR *pAddr, int NumAddrs)
{
	m_Connection.Connect7(pAddr, NumAddrs);
}

void CNeBestClient::ResetErrorString()
{
	m_Connection.ResetErrorString();
}

int CNeBestClient::Recv(CNetChunk *pChunk, SECURITY_TOKEN *pResponseToken, bool Sixup)
{
	while(true)
	{
		// check for a chunk
...
int CNeBestClient::Send(CNetChunk *pChunk)
{
	if(pChunk->m_DataSize >= NET_MAX_PAYLOAD)
	{
		dbg_msg("neBestClient", "chunk payload too big. %d. dropping chunk", pChunk->m_DataSize);
		return -1;
	}
...
int CNeBestClient::Flush()
{
	return m_Connection.Flush();
}

bool CNeBestClient::GotProblems(int64_t MaxLatency) const
{
	return time_get() - m_Connection.LastRecvTime() > MaxLatency;
}

const char *CNeBestClient::ErrorString() const
{
	return m_Connection.ErrorString();
}

void CNeBestClient::FeedStunServer(NETADDR StunServer)
{
	m_pStun->FeedStunServer(StunServer);
}

void CNeBestClient::RefreshStun()
{
	m_pStun->Refresh();
}

CONNECTIVITY CNeBestClient::GetConnectivity(int NetType, NETADDR *pGlobalAddr)
{
