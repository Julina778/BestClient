...
public:
	...
	// Needed for GotProblems in NeBestClient
...
class CNetServer
{
	...
	bool ClientExists(const NETADDR &Addr) { return GeBestClientSlot(Addr) != -1; }
	int GeBestClientSlot(const NETADDR &Addr);
	void SendControl(NETADDR &Addr, int ControlMsg, const void *pExtra, int ExtraSize, SECURITY_TOKEN SecurityToken);

	int TryAccepBestClient(NETADDR &Addr, SECURITY_TOKEN SecurityToken, bool VanillaAuth = false, bool Sixup = false, SECURITY_TOKEN Token = 0);
...
public:
	...
	//
	int AccepBestClient(NETSOCKET Socket, const NETADDR *pAddr);
	void Drop(int ClientId, const char *pReason);
