...
class CHttpRequest : public IHttpRequest
{
	...
	// 256 bytes is too small for some valid URLs (e.g. translation backends with URL-encoded query text).
	// Keep this reasonably sized to avoid truncation bugs while still staying stack/queue friendly.
	char m_aUrl[2048] = {0};
	...
	// BestClient
	bool m_VerifyPeer = true; // for SSL check
...
public:
	...
	// BestClient
	void VerifyPeer(bool VerifyPeer) { m_VerifyPeer = VerifyPeer; }
