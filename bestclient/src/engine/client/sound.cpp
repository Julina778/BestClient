...
#endif
	// Ensure SDL exposes loopback/monitor capture devices (system output) to our visualizers.
...
bool CSound::DecodeWV(CSample &Sample, const void *pData, unsigned DataSize, const char *pContextName) const
{
	...
	// Accept regular RIFF/WAVE files in addition to WavPack data.
	if(DataSize >= 12 && mem_comp(pData, "RIFF", 4) == 0 && mem_comp((const char *)pData + 8, "WAVE", 4) == 0)
	{
		SDL_AudioSpec Spec = {};
		Uint8 *pWavBuffer = nullptr;
		Uint32 WavBufferLen = 0;
		SDL_RWops *pRw = SDL_RWFromConstMem(pData, (int)DataSize);
		if(!pRw)
		{
			log_error("sound/wav", "Failed to create SDL RWops. Filename='%s' Error='%s'", pContextName, SDL_GetError());
			return false;
		}

		if(SDL_LoadWAV_RW(pRw, 1, &Spec, &pWavBuffer, &WavBufferLen) == nullptr)
		{
			log_error("sound/wav", "Failed to decode wav. Filename='%s' Error='%s'", pContextName, SDL_GetError());
			return false;
		}

		if(Spec.channels < 1 || Spec.channels > 2)
		{
			log_error("sound/wav", "File is not mono or stereo. Filename='%s'", pContextName);
			SDL_FreeWAV(pWavBuffer);
			return false;
		}

		SDL_AudioCVT Cvt = {};
		const int BuildResult = SDL_BuildAudioCVT(&Cvt, Spec.format, Spec.channels, Spec.freq, AUDIO_S16SYS, Spec.channels, Spec.freq);
		if(BuildResult < 0)
		{
			log_error("sound/wav", "Failed to setup audio conversion. Filename='%s' Error='%s'", pContextName, SDL_GetError());
			SDL_FreeWAV(pWavBuffer);
			return false;
		}

		const Uint8 *pDecodedData = pWavBuffer;
		Uint32 DecodedDataSize = WavBufferLen;
		if(Cvt.needed)
		{
			Cvt.len = (int)WavBufferLen;
			Cvt.buf = (Uint8 *)SDL_malloc((size_t)Cvt.len * Cvt.len_mult);
			if(!Cvt.buf)
			{
				log_error("sound/wav", "Failed to allocate conversion buffer. Filename='%s'", pContextName);
				SDL_FreeWAV(pWavBuffer);
				return false;
			}
			mem_copy(Cvt.buf, pWavBuffer, WavBufferLen);

			if(SDL_ConvertAudio(&Cvt) < 0)
			{
				log_error("sound/wav", "Failed to convert wav data. Filename='%s' Error='%s'", pContextName, SDL_GetError());
				SDL_free(Cvt.buf);
				SDL_FreeWAV(pWavBuffer);
				return false;
			}

			pDecodedData = Cvt.buf;
			DecodedDataSize = (Uint32)Cvt.len_cvt;
		}

		const int BytesPerFrame = (int)(sizeof(short) * Spec.channels);
		const int NumFrames = (int)(DecodedDataSize / (Uint32)BytesPerFrame);
		if(NumFrames <= 0)
		{
			log_error("sound/wav", "Decoded wav has no audio frames. Filename='%s'", pContextName);
			if(Cvt.needed)
				SDL_free(Cvt.buf);
			SDL_FreeWAV(pWavBuffer);
			return false;
		...
		Sample.m_pData = (short *)calloc((size_t)NumFrames * Spec.channels, sizeof(short));
		mem_copy(Sample.m_pData, pDecodedData, (size_t)NumFrames * Spec.channels * sizeof(short));
		Sample.m_NumFrames = NumFrames;
		Sample.m_Rate = Spec.freq;
		Sample.m_Channels = Spec.channels;
		Sample.m_LoopStart = -1;
		Sample.m_LoopEnd = -1;
		Sample.m_PausedAt = 0;
