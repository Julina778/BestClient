...
namespace TextureFlag
{
	...
	inline constexpr uint32_t RENDER_TARGET = 1 << 4;
};
...
public:
	...
	enum ECommandBufferCMD
	{
		...
		CMD_RENDER,
		CMD_RENDER_TEX3D,
		CMD_RENDER_TARGET_CAPTURE_SCREEN,
		CMD_RENDER_TARGET_BIND,
		CMD_RENDER_TARGET_UNBIND,
		CMD_RENDER_POST_PROCESS,
	...
	struct SCommand_RenderTarget_Bind : public SCommand
	{
		SCommand_RenderTarget_Bind() :
			SCommand(CMD_RENDER_TARGET_BIND) {}
		int m_Slot = -1;
	};

	struct SCommand_RenderTarget_CaptureScreen : public SCommand
	{
		SCommand_RenderTarget_CaptureScreen() :
			SCommand(CMD_RENDER_TARGET_CAPTURE_SCREEN) {}
		int m_Slot = -1;
	};

	struct SCommand_RenderTarget_Unbind : public SCommand
	{
		SCommand_RenderTarget_Unbind() :
			SCommand(CMD_RENDER_TARGET_UNBIND) {}
	};

	struct SCommand_RenderPostProcess : public SCommand
	{
		SCommand_RenderPostProcess() :
			SCommand(CMD_RENDER_POST_PROCESS) {}
		int m_DstSlot = -1;
		int m_SrcSlot0 = -1;
		int m_SrcSlot1 = -1;
		IGraphics::EPostProcessShader m_Shader;
		IGraphics::CPostProcessParams m_Params;
	};
...
public:
	...
	IGraphics::CTextureHandle RenderTargetHandleToTexture(IGraphics::CRenderTargetHandle RenderTarget) const
	{
		return const_cast<CGraphics_Threaded *>(this)->CreateTextureHandle(RenderTarget.Id());
	}
	...
	bool UpdateTextureRaw(CTextureHandle TextureId, int x, int y, size_t Width, size_t Height, const CImageInfo &Image, bool IsMovedPointer) override;
	CRenderTargetHandle CreateRenderTarget(uint32_t Width, uint32_t Height) override;
	void DeleteRenderTarget(CRenderTargetHandle *pRenderTarget) override;
	CTextureHandle GetRenderTargetTexture(CRenderTargetHandle RenderTarget) override;
	void CaptureScreenToRenderTarget(CRenderTargetHandle RenderTarget) override;
	void BeginRenderTarget(CRenderTargetHandle RenderTarget) override;
	void EndRenderTarget() override;
	void RenderPostProcess(CRenderTargetHandle DstRenderTarget, EPostProcessShader Shader, CTextureHandle SrcTexture0, CTextureHandle SrcTexture1, const CPostProcessParams &Params) override;
