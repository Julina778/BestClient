...
void CServerBrowser::SetBaseInfo(class CNeBestClient *pClient, const char *pNetVersion)
{
	m_pNeBestClient = pClient;
...
void CServerBrowser::SetBestClientPlayers(const std::vector<CBestClientPlayerEntry> &vPlayers)
{
	...
	for(int i = 0; i < m_NumServers; ++i)
		UpdateServerBestClients(&m_ppServerlist[i]->m_Info);
...
void CServerBrowser::Sort()
{
	...
	if(g_Config.m_BrSortOrder == 2 && (g_Config.m_BrSort == IServerBrowser::SORT_NUMPLAYERS || g_Config.m_BrSort == IServerBrowser::SORT_PING))
		...
		std::stable_sort(m_pSortedServerlist, m_pSortedServerlist + m_NumSortedServers, CSortWrap(this, &CServerBrowser::SortCompareNumBestClientPlayers));
...
void CServerBrowser::OnServerInfoUpdate(const NETADDR &Addr, int Token, const CServerInfo *pInfo)
{
	...
	if(m_ServerlistType == IServerBrowser::TYPE_LAN)
	{
		...
		Broadcast.type = (m_pNeBestClient->NetType() & ~(NETTYPE_WEBSOCKET_IPV4 | NETTYPE_WEBSOCKET_IPV6)) | NETTYPE_LINK_BROADCAST;
...
void CServerBrowser::Refresh(int Type, bool Force)
{
	...
	if(Type == IServerBrowser::TYPE_LAN)
	{
		...
		Packet.m_Address.type = (m_pNeBestClient->NetType() & ~(NETTYPE_WEBSOCKET_IPV4 | NETTYPE_WEBSOCKET_IPV6)) | NETTYPE_LINK_BROADCAST;
		...
		Packet7.m_Address.type = (m_pNeBestClient->NetType() & ~(NETTYPE_WEBSOCKET_IPV4 | NETTYPE_WEBSOCKET_IPV6)) | NETTYPE_TW7 | NETTYPE_LINK_BROADCAST;
		...
		for(int Port = LAN_PORT_BEGIN; Port <= LAN_PORT_END; Port++)
		{
			...
			m_pNeBestClient->Send(&Packet);

			Packet7.m_Address.port = Port;
			m_pNeBestClient->Send(&Packet7);
		}
