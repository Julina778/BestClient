...
class CCommandProcessorFragment_Vulkan : public CCommandProcessorFragment_GLBase
{
	...
	struct SWorldEditorPushConstants
	{
		std::array<float, 4> m_ParamsA{};
		std::array<float, 4> m_ParamsB{};
		std::array<float, 2> m_TexelSize0{};
		std::array<float, 2> m_TexelSize1{};
	};
...
private:
	...
	SPipelineContainer m_WorldEditorPipeline;

	std::vector<VkPipeline> m_vLastPipeline;
	std::vector<SDeviceDescriptorSet> m_vWorldEditorDescriptorSets;

	std::vector<VkCommandPool> m_vCommandPools;

	VkRenderPass m_VKRenderPass;
	VkRenderPass m_VKLoadRenderPass = VK_NULL_HANDLE;
	VkRenderPass m_VKActiveRenderPass = VK_NULL_HANDLE;
	bool m_MainRenderPassActive = false;
...
protected:
	...
	void RegisterCommands()
	{
		...
		m_aCommandCallbacks[CommandBufferCMDOff(CCommandBuffer::CMD_RENDER_TARGET_CAPTURE_SCREEN)] = {false, [](SRenderCommandExecuteBuffer &ExecBuffer, const CCommandBuffer::SCommand *pBaseCommand) {}, [this](const CCommandBuffer::SCommand *pBaseCommand, SRenderCommandExecuteBuffer &ExecBuffer) { return Cmd_RenderTarget_CaptureScreen(static_cast<const CCommandBuffer::SCommand_RenderTarget_CaptureScreen *>(pBaseCommand)); }};
		m_aCommandCallbacks[CommandBufferCMDOff(CCommandBuffer::CMD_RENDER_POST_PROCESS)] = {false, [](SRenderCommandExecuteBuffer &ExecBuffer, const CCommandBuffer::SCommand *pBaseCommand) {}, [this](const CCommandBuffer::SCommand *pBaseCommand, SRenderCommandExecuteBuffer &ExecBuffer) { return Cmd_RenderPostProcess(static_cast<const CCommandBuffer::SCommand_RenderPostProcess *>(pBaseCommand)); }};
	...
	void ClearFrameData(size_t FrameImageIndex)
	{
		...
		if(FrameImageIndex < m_vWorldEditorDescriptorSets.size())
		{
			auto &DescrSet = m_vWorldEditorDescriptorSets[FrameImageIndex];
	...
	[[nodiscard]] bool PrepareFrame()
	{
		...
		BeginMainRenderPass(m_VKRenderPass);
	...
	void ExecutePendingGraphicCommandBuffers(VkCommandBuffer &CommandBuffer)
	{
		if(m_ThreadCount > 1)
		{
	...
	void BeginMainRenderPass(VkRenderPass RenderPass)
	{
		auto &CommandBuffer = GetMainGraphicCommandBuffer();
		...
		m_VKActiveRenderPass = RenderPass;
		m_MainRenderPassActive = true;
	}

	void EndMainRenderPass()
	{
		if(!m_MainRenderPassActive)
			return;
		auto &CommandBuffer = GetMainGraphicCommandBuffer();
		ExecutePendingGraphicCommandBuffers(CommandBuffer);
		vkCmdEndRenderPass(CommandBuffer);
		for(auto &LastPipe : m_vLastPipeline)
			LastPipe = VK_NULL_HANDLE;
		m_VKActiveRenderPass = VK_NULL_HANDLE;
		m_MainRenderPassActive = false;
	}
	...
	[[nodiscard]] bool CreateTextureImage(size_t ImageIndex, VkImage &NewImage, SMemoryImageBlock<IMAGE_BUFFER_CACHE_ID> &NewImgMem, const uint8_t *pData, VkFormat Format, size_t Width, size_t Height, size_t Depth, size_t PixelSize, size_t MipMapLevelCount)
	{
		...
		if(pData == nullptr)
		{
			// Render targets are created without initial upload data. Keep them sampleable so the
			// first capture/postprocess pass can transition them from a known layout.
			if(!ImageBarrier(NewImage, 0, MipMapLevelCount, 0, Depth, ImgFormat, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL))
				return false;
			return true;
		}
	...
	[[nodiscard]] bool ImageBarrierImpl(VkCommandBuffer &CommandBuffer, const VkImage &Image, size_t MipMapBase, size_t MipMapCount, size_t LayerBase, size_t LayerCount, VkFormat Format, VkImageLayout OldLayout, VkImageLayout NewLayout)
	{
		VkImageMemoryBarrier Barrier{};
		Barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
	...
	[[nodiscard]] bool ImageBarrier(const VkImage &Image, size_t MipMapBase, size_t MipMapCount, size_t LayerBase, size_t LayerCount, VkFormat Format, VkImageLayout OldLayout, VkImageLayout NewLayout)
	{
		...
		if(!GetMemoryCommandBuffer(pMemCommandBuffer))
			return false;
		return ImageBarrierImpl(*pMemCommandBuffer, Image, MipMapBase, MipMapCount, LayerBase, LayerCount, Format, OldLayout, NewLayout);
	}
...
public:
	...
	SSwapImgViewportExtent GetSwapImageSize(const VkSurfaceCapabilitiesKHR &VKCapabilities)
	{
		...
		const bool HasCustomAspect = AspectMode > 0 && g_Config.m_BcCustomAspectRatio >= 100;
	...
	[[nodiscard]] bool CreateRenderPass(bool ClearAttachments, VkRenderPass &RenderPass, bool LoadPresentedImage = false)
	{
		...
		MultiSamplingColorAttachment.initialLayout = LoadPresentedImage ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_UNDEFINED;
		...
		if(LoadPresentedImage)
			ColorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_LOAD;
		else if(ClearAttachments && !HasMultiSamplingTargets)
		...
		ColorAttachment.initialLayout = LoadPresentedImage ? VK_IMAGE_LAYOUT_PRESENT_SRC_KHR : VK_IMAGE_LAYOUT_UNDEFINED;
	...
	void DestroyRenderPass()
	{
		...
		if(m_VKLoadRenderPass != VK_NULL_HANDLE)
			vkDestroyRenderPass(m_VKDevice, m_VKLoadRenderPass, nullptr);
		m_VKRenderPass = VK_NULL_HANDLE;
		m_VKLoadRenderPass = VK_NULL_HANDLE;
	}
		...
		EVulkanBackendBlendModes BlendMode, EVulkanBackendClipModes DynamicMode, bool IsLinePrim = false)
	{
		...
		VertexInputInfo.vertexBindingDescriptionCount = aInputAttr.empty() ? 0 : 1;
		VertexInputInfo.vertexAttributeDescriptionCount = aInputAttr.size();
		VertexInputInfo.pVertexBindingDescriptions = aInputAttr.empty() ? nullptr : &BindingDescription;
	...
	[[nodiscard]] bool CreateWorldEditorGraphicsPipeline(const char *pVertName, const char *pFragName)
	{
		std::array<VkVertexInputAttributeDescription, 0> aAttributeDescriptions = {};
		std::array<VkDescriptorSetLayout, 1> aSetLayouts = {m_TextDescriptorSetLayout};
		std::array<VkPushConstantRange, 1> aPushConstants = {
			VkPushConstantRange{VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(SWorldEditorPushConstants)}};
		return CreateGraphicsPipeline<true>(pVertName, pFragName, m_WorldEditorPipeline, 0, aAttributeDescriptions, aSetLayouts, aPushConstants,
			VULKAN_BACKEND_TEXTURE_MODE_TEXTURED, VULKAN_BACKEND_BLEND_MODE_NONE, VULKAN_BACKEND_CLIP_MODE_DYNAMIC_SCISSOR_AND_VIEWPORT);
	}
	...
	void CleanupVulkanSwapChain(bool ForceSwapChainDestruct)
	{
		...
		m_WorldEditorPipeline.Destroy(m_VKDevice);
	...
	[[nodiscard]] bool CreateWorldEditorDescriptorSet(size_t Texture0, size_t Texture1, SDeviceDescriptorSet &DescrSet)
	{
		VkDescriptorSetAllocateInfo DesAllocInfo{};
		DesAllocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
		if(!GetDescriptorPoolForAlloc(DesAllocInfo.descriptorPool, m_TextTextureDescrPool, &DescrSet, 1))
			return false;
		DesAllocInfo.descriptorSetCount = 1;
		DesAllocInfo.pSetLayouts = &m_TextDescriptorSetLayout;
		...
		const auto &Src0 = m_vTextures[Texture0];
		const auto &Src1 = m_vTextures[Texture1];
		std::array<VkDescriptorImageInfo, 2> aImageInfo{};
		aImageInfo[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		aImageInfo[0].imageView = Src0.m_ImgView;
		aImageInfo[0].sampler = Src0.m_aSamplers[1];
		aImageInfo[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		aImageInfo[1].imageView = Src1.m_ImgView;
		aImageInfo[1].sampler = Src1.m_aSamplers[1];
		...
		aDescriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
		aDescriptorWrites[0].descriptorCount = 1;
		aDescriptorWrites[0].pImageInfo = &aImageInfo[0];
		aDescriptorWrites[1] = aDescriptorWrites[0];
		aDescriptorWrites[1].dstBinding = 1;
		aDescriptorWrites[1].pImageInfo = &aImageInfo[1];
		vkUpdateDescriptorSets(m_VKDevice, static_cast<uint32_t>(aDescriptorWrites.size()), aDescriptorWrites.data(), 0, nullptr);
	...
	int InitVulkanSwapChain(VkSwapchainKHR &OldSwapChain)
	{
		...
		if(!CreateRenderPass(true, m_VKRenderPass))
			return -1;

		if(!CreateRenderPass(false, m_VKLoadRenderPass, true))
			return -1;
		...
		if(!CreateWorldEditorGraphicsPipeline("shader/vulkan/world_editor.vert.spv", "shader/vulkan/world_editor.frag.spv"))
			return -1;
	...
	int InitVulkan()
	{
		...
		m_vWorldEditorDescriptorSets.resize(m_SwapChainImageCount);
	...
	[[nodiscard]] bool GetGraphicCommandBuffer(VkCommandBuffer *&pDrawCommandBuffer, size_t RenderThreadIndex)
	{
		...
		else
		{
			...
			if(!m_vvUsedThreadDrawCommandBuffer[RenderThreadIndex][m_CurImageIndex])
			{
				...
				InheritanceInfo.renderPass = m_VKActiveRenderPass != VK_NULL_HANDLE ? m_VKActiveRenderPass : m_VKRenderPass;
	...
	[[nodiscard]] bool Cmd_Update_Viewport(const CCommandBuffer::SCommand_Update_Viewport *pCommand)
	{
		...
		{
			auto Viewport = m_VKSwapImgAndViewportExtent.m_SwapImageViewport;
	...
	[[nodiscard]] bool Cmd_RenderTarget_CaptureScreen(const CCommandBuffer::SCommand_RenderTarget_CaptureScreen *pCommand)
	{
		if(pCommand->m_Slot < 0 || pCommand->m_Slot >= (int)m_vTextures.size())
			return true;

		auto &DstTex = m_vTextures[pCommand->m_Slot];
		if(DstTex.m_Img == VK_NULL_HANDLE)
			return true;

		EndMainRenderPass();

		auto &CommandBuffer = GetMainGraphicCommandBuffer();
		auto &SwapImg = m_vSwapChainImages[m_CurImageIndex];

		if(!ImageBarrierImpl(CommandBuffer, DstTex.m_Img, 0, 1, 0, 1, DstTex.m_Format, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL))
			return false;
		if(!ImageBarrierImpl(CommandBuffer, SwapImg, 0, 1, 0, 1, m_VKSurfFormat.format, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL))
			return false;

		const bool CanBlit = m_OptimalSwapChainImageBlitting && DstTex.m_Format == VK_FORMAT_R8G8B8A8_UNORM && m_OptimalRGBAImageBlitting;
		if(CanBlit)
		{
			VkImageBlit ImageBlitRegion{};
			ImageBlitRegion.srcSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
			ImageBlitRegion.srcSubresource.layerCount = 1;
			ImageBlitRegion.srcOffsets[1] = {(int32_t)DstTex.m_Width, (int32_t)DstTex.m_Height, 1};
			ImageBlitRegion.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
			ImageBlitRegion.dstSubresource.layerCount = 1;
			ImageBlitRegion.dstOffsets[1] = {(int32_t)DstTex.m_Width, (int32_t)DstTex.m_Height, 1};
			vkCmdBlitImage(CommandBuffer, SwapImg, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, DstTex.m_Img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &ImageBlitRegion, VK_FILTER_NEAREST);
		}
		else if(m_VKSurfFormat.format == DstTex.m_Format)
		{
			VkImageCopy ImageCopyRegion{};
			ImageCopyRegion.srcSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
			ImageCopyRegion.srcSubresource.layerCount = 1;
			ImageCopyRegion.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
			ImageCopyRegion.dstSubresource.layerCount = 1;
			ImageCopyRegion.extent.width = DstTex.m_Width;
			ImageCopyRegion.extent.height = DstTex.m_Height;
			ImageCopyRegion.extent.depth = 1;
			vkCmdCopyImage(CommandBuffer, SwapImg, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, DstTex.m_Img, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &ImageCopyRegion);
		}
		else
		{
			SetError(EGfxErrorType::GFX_ERROR_TYPE_RENDER_RECORDING, "World Editor Vulkan capture requires swapchain blit support.");
			return false;
		}

		if(!ImageBarrierImpl(CommandBuffer, DstTex.m_Img, 0, 1, 0, 1, DstTex.m_Format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL))
			return false;
		if(!ImageBarrierImpl(CommandBuffer, SwapImg, 0, 1, 0, 1, m_VKSurfFormat.format, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR))
			return false;

		BeginMainRenderPass(m_VKLoadRenderPass);
		return true;
	}

	[[nodiscard]] bool Cmd_RenderPostProcess(const CCommandBuffer::SCommand_RenderPostProcess *pCommand)
	{
		if(pCommand->m_Shader != IGraphics::EPostProcessShader::WORLD_EDITOR_COMBINED)
			return true;
		if(pCommand->m_SrcSlot0 < 0 || pCommand->m_SrcSlot0 >= (int)m_vTextures.size())
			return true;

		const size_t Src0Index = (size_t)pCommand->m_SrcSlot0;
		const size_t Src1Index = pCommand->m_SrcSlot1 >= 0 && pCommand->m_SrcSlot1 < (int)m_vTextures.size() ? (size_t)pCommand->m_SrcSlot1 : Src0Index;
		auto &FrameDescrSet = m_vWorldEditorDescriptorSets[m_CurImageIndex];
		if(FrameDescrSet.m_Descriptor == VK_NULL_HANDLE)
		{
			if(!CreateWorldEditorDescriptorSet(Src0Index, Src1Index, FrameDescrSet))
				return false;
		}
		...
		auto &PipeLayout = GetPipeLayout(m_WorldEditorPipeline, true, VULKAN_BACKEND_BLEND_MODE_NONE, VULKAN_BACKEND_CLIP_MODE_DYNAMIC_SCISSOR_AND_VIEWPORT);
		auto &PipeLine = GetPipeline(m_WorldEditorPipeline, true, VULKAN_BACKEND_BLEND_MODE_NONE, VULKAN_BACKEND_CLIP_MODE_DYNAMIC_SCISSOR_AND_VIEWPORT);
		BindPipeline(0, CommandBuffer, ExecBuffer, PipeLine, State);
		vkCmdBindDescriptorSets(CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, PipeLayout, 0, 1, &FrameDescrSet.m_Descriptor, 0, nullptr);

		SWorldEditorPushConstants PushConstants{};
		for(size_t i = 0; i < 4; ++i)
			PushConstants.m_ParamsA[i] = pCommand->m_Params.m_aValues[i];
		for(size_t i = 0; i < 4; ++i)
			PushConstants.m_ParamsB[i] = pCommand->m_Params.m_aValues[i + 4];

		const auto &Src0 = m_vTextures[Src0Index];
		const auto &Src1 = m_vTextures[Src1Index];
		PushConstants.m_TexelSize0[0] = Src0.m_Width > 0 ? 1.0f / Src0.m_Width : 0.0f;
		PushConstants.m_TexelSize0[1] = Src0.m_Height > 0 ? 1.0f / Src0.m_Height : 0.0f;
		PushConstants.m_TexelSize1[0] = Src1.m_Width > 0 ? 1.0f / Src1.m_Width : 0.0f;
		PushConstants.m_TexelSize1[1] = Src1.m_Height > 0 ? 1.0f / Src1.m_Height : 0.0f;
