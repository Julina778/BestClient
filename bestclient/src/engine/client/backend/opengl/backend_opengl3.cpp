...
bool CCommandProcessorFragment_OpenGL3_3::Cmd_Init(const SCommand_Init *pCommand)
{
	...
	m_WorldEditorProgram.m_pProgram = new CGLSLProgram;
	m_LastProgramId = 0;
	...
	}
	{
		...
		VertexShader.LoadShader(&ShaderCompiler, pCommand->m_pStorage, "shader/world_editor.vert", GL_VERTEX_SHADER);
		FragmentShader.LoadShader(&ShaderCompiler, pCommand->m_pStorage, "shader/world_editor.frag", GL_FRAGMENT_SHADER);

		m_WorldEditorProgram.m_pProgram->CreateProgram();
		m_WorldEditorProgram.m_pProgram->AddShader(&VertexShader);
		m_WorldEditorProgram.m_pProgram->AddShader(&FragmentShader);
		m_WorldEditorProgram.m_pProgram->LinkProgram();
		m_WorldEditorProgram.m_pProgram->UseProgram();
		m_WorldEditorProgram.m_LocTexture0 = m_WorldEditorProgram.m_pProgram->GetUniformLoc("gTextureSampler");
		m_WorldEditorProgram.m_LocTexture1 = m_WorldEditorProgram.m_pProgram->GetUniformLoc("gTextureSampler2");
		m_WorldEditorProgram.m_LocParams = m_WorldEditorProgram.m_pProgram->GetUniformLoc("gParams");
		m_WorldEditorProgram.m_LocTexelSize0 = m_WorldEditorProgram.m_pProgram->GetUniformLoc("gTexelSize0");
		m_WorldEditorProgram.m_LocTexelSize1 = m_WorldEditorProgram.m_pProgram->GetUniformLoc("gTexelSize1");
		m_WorldEditorProgram.m_LocPassIndex = m_WorldEditorProgram.m_pProgram->GetUniformLoc("gPassIndex");
		m_WorldEditorProgram.m_pProgram->SetUniform(m_WorldEditorProgram.m_LocTexture0, 0);
		m_WorldEditorProgram.m_pProgram->SetUniform(m_WorldEditorProgram.m_LocTexture1, 1);
	}
	...
	glGenVertexArrays(1, &m_WorldEditorVao);
...
void CCommandProcessorFragment_OpenGL3_3::Cmd_Shutdown(const SCommand_Shutdown *pCommand)
{
	...
	m_WorldEditorProgram.m_pProgram->DeleteProgram();
	...
	delete m_WorldEditorProgram.m_pProgram;
	...
	glDeleteVertexArrays(1, &m_WorldEditorVao);
...
void CCommandProcessorFragment_OpenGL3_3::TextureCreate(int Slot, int Width, int Height, int GLFormat, int GLStoreFormat, int Flags, uint8_t *pTexData)
{
	...
	m_vTextures[Slot].m_IsRenderTarget = (Flags & TextureFlag::RENDER_TARGET) != 0;

	if(m_vTextures[Slot].m_IsRenderTarget)
	{
		glGenFramebuffers(1, &m_vTextures[Slot].m_FrameBufferObject);
		glBindFramebuffer(GL_FRAMEBUFFER, m_vTextures[Slot].m_FrameBufferObject);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_vTextures[Slot].m_Tex, 0);
		const GLenum Status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
		dbg_assert(Status == GL_FRAMEBUFFER_COMPLETE, "OpenGL render target framebuffer is incomplete");
		BindRenderTargetFramebuffer(m_CurrentRenderTarget);
	}
...
void CCommandProcessorFragment_OpenGL3_3::Cmd_RenderPostProcess(const CCommandBuffer::SCommand_RenderPostProcess *pCommand)
{
	if(pCommand->m_SrcSlot0 < 0 || pCommand->m_SrcSlot0 >= (int)m_vTextures.size() || m_WorldEditorProgram.m_pProgram == nullptr)
		return;

	const auto &Src0 = m_vTextures[pCommand->m_SrcSlot0];
	if(Src0.m_Tex == 0)
		return;

	const bool HasSrc1 = pCommand->m_SrcSlot1 >= 0 && pCommand->m_SrcSlot1 < (int)m_vTextures.size() && m_vTextures[pCommand->m_SrcSlot1].m_Tex != 0;
	const auto &Src1 = HasSrc1 ? m_vTextures[pCommand->m_SrcSlot1] : m_vTextures[pCommand->m_SrcSlot0];

	BindRenderTargetFramebuffer(pCommand->m_DstSlot);

	glDisable(GL_BLEND);
	glDisable(GL_SCISSOR_TEST);

	m_WorldEditorProgram.m_pProgram->UseProgram();
	m_LastProgramId = m_WorldEditorProgram.m_pProgram->GetProgramId();

	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, Src0.m_Tex);
	glBindSampler(0, Src0.m_Sampler);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, HasSrc1 ? Src1.m_Tex : 0);
	glBindSampler(1, HasSrc1 ? Src1.m_Sampler : 0);

	const float aTexelSize0[2] = {
		Src0.m_Width > 0 ? 1.0f / Src0.m_Width : 0.0f,
		Src0.m_Height > 0 ? 1.0f / Src0.m_Height : 0.0f,
	};
	const float aTexelSize1[2] = {
		Src1.m_Width > 0 ? 1.0f / Src1.m_Width : 0.0f,
		Src1.m_Height > 0 ? 1.0f / Src1.m_Height : 0.0f,
	};

	m_WorldEditorProgram.m_pProgram->SetUniform(m_WorldEditorProgram.m_LocPassIndex, (int)pCommand->m_Shader);
	m_WorldEditorProgram.m_pProgram->SetUniform(m_WorldEditorProgram.m_LocParams, std::size(pCommand->m_Params.m_aValues), pCommand->m_Params.m_aValues);
	m_WorldEditorProgram.m_pProgram->SetUniformVec2(m_WorldEditorProgram.m_LocTexelSize0, 1, aTexelSize0);
	m_WorldEditorProgram.m_pProgram->SetUniformVec2(m_WorldEditorProgram.m_LocTexelSize1, 1, aTexelSize1);

	glBindVertexArray(m_WorldEditorVao);
