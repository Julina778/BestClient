...
{
	m_DefaultViewportX = pCommand->m_X;
	m_DefaultViewportY = pCommand->m_Y;
	m_DefaultViewportW = pCommand->m_Width;
	m_DefaultViewportH = pCommand->m_Height;
...
void CCommandProcessorFragment_OpenGL::BindRenderTargetFramebuffer(int Slot)
{
	if(Slot >= 0 && Slot < (int)m_vTextures.size() && m_vTextures[Slot].m_FrameBufferObject != 0)
	{
		glBindFramebuffer(GL_FRAMEBUFFER, m_vTextures[Slot].m_FrameBufferObject);
		glViewport(0, 0, m_vTextures[Slot].m_Width, m_vTextures[Slot].m_Height);
		m_CurrentRenderTarget = Slot;
	}
	else
	{
		glBindFramebuffer(GL_FRAMEBUFFER, 0);
		const int ViewportW = m_DefaultViewportW > 0 ? m_DefaultViewportW : (int)m_CanvasWidth;
		const int ViewportH = m_DefaultViewportH > 0 ? m_DefaultViewportH : (int)m_CanvasHeight;
		glViewport(m_DefaultViewportX, m_DefaultViewportY, ViewportW, ViewportH);
...
void CCommandProcessorFragment_OpenGL::DestroyTexture(int Slot)
{
	...
	if(m_vTextures[Slot].m_FrameBufferObject != 0)
	{
		glDeleteFramebuffers(1, &m_vTextures[Slot].m_FrameBufferObject);
	}
	...
	m_vTextures[Slot].m_FrameBufferObject = 0;
	m_vTextures[Slot].m_LastWrapMode = EWrapMode::REPEAT;
	m_vTextures[Slot].m_IsRenderTarget = false;
}
...
void CCommandProcessorFragment_OpenGL::Cmd_RenderTarget_Bind(const CCommandBuffer::SCommand_RenderTarget_Bind *pCommand)
{
	BindRenderTargetFramebuffer(pCommand->m_Slot);
}

void CCommandProcessorFragment_OpenGL::Cmd_RenderTarget_CaptureScreen(const CCommandBuffer::SCommand_RenderTarget_CaptureScreen *pCommand)
{
	if(pCommand->m_Slot < 0 || pCommand->m_Slot >= (int)m_vTextures.size())
		return;
	if(m_vTextures[pCommand->m_Slot].m_Tex == 0)
		return;

	glBindFramebuffer(GL_FRAMEBUFFER, 0);
	glBindTexture(GL_TEXTURE_2D, m_vTextures[pCommand->m_Slot].m_Tex);
	glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, m_DefaultViewportX, m_DefaultViewportY, m_vTextures[pCommand->m_Slot].m_Width, m_vTextures[pCommand->m_Slot].m_Height);
}

void CCommandProcessorFragment_OpenGL::Cmd_RenderTarget_Unbind(const CCommandBuffer::SCommand_RenderTarget_Unbind *pCommand)
{
	(void)pCommand;
	BindRenderTargetFramebuffer(-1);
}
...
ERunCommandReturnTypes CCommandProcessorFragment_OpenGL::RunCommand(const CCommandBuffer::SCommand *pBaseCommand)
{
	...
	case CCommandBuffer::CMD_RENDER_TARGET_BIND:
		Cmd_RenderTarget_Bind(static_cast<const CCommandBuffer::SCommand_RenderTarget_Bind *>(pBaseCommand));
		break;
	case CCommandBuffer::CMD_RENDER_TARGET_CAPTURE_SCREEN:
		Cmd_RenderTarget_CaptureScreen(static_cast<const CCommandBuffer::SCommand_RenderTarget_CaptureScreen *>(pBaseCommand));
		break;
	case CCommandBuffer::CMD_RENDER_TARGET_UNBIND:
		Cmd_RenderTarget_Unbind(static_cast<const CCommandBuffer::SCommand_RenderTarget_Unbind *>(pBaseCommand));
		break;
