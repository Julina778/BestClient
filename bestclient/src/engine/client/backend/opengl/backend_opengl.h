...
protected:
	...
	{
		CTexture() :
			m_Tex(0), m_Tex2DArray(0), m_FrameBufferObject(0), m_Sampler(0), m_Sampler2DArray(0), m_LastWrapMode(EWrapMode::REPEAT), m_IsRenderTarget(false), m_MemSize(0), m_Width(0), m_Height(0), m_RescaleCount(0), m_ResizeWidth(0), m_ResizeHeight(0)
		{
		}

		TWGLuint m_Tex;
		TWGLuint m_Tex2DArray; // or 3D texture as fallback
		TWGLuint m_FrameBufferObject;
		TWGLuint m_Sampler;
	...
	int m_DefaultViewportX = 0;
	int m_DefaultViewportY = 0;
	int m_DefaultViewportW = 0;
	int m_DefaultViewportH = 0;

	bool IsTexturedState(const CCommandBuffer::SState &State);
	void BindRenderTargetFramebuffer(int Slot);
	...
	virtual void Cmd_RenderTarget_CaptureScreen(const CCommandBuffer::SCommand_RenderTarget_CaptureScreen *pCommand);
	virtual void Cmd_RenderTarget_Bind(const CCommandBuffer::SCommand_RenderTarget_Bind *pCommand);
	virtual void Cmd_RenderTarget_Unbind(const CCommandBuffer::SCommand_RenderTarget_Unbind *pCommand);
	virtual void Cmd_RenderPostProcess(const CCommandBuffer::SCommand_RenderPostProcess *pCommand) {}
