...
IGraphics::CRenderTargetHandle CGraphics_Threaded::CreateRenderTarget(uint32_t Width, uint32_t Height)
{
	if(Width == 0 || Height == 0)
		return IGraphics::CRenderTargetHandle();

	IGraphics::CTextureHandle TextureHandle = FindFreeTextureIndex();
	CCommandBuffer::SCommand_Texture_Create Cmd = LoadTextureCreateCommand(TextureHandle.Id(), Width, Height, 0);
	Cmd.m_Flags |= TextureFlag::NO_MIPMAPS;
	Cmd.m_Flags |= TextureFlag::RENDER_TARGET;
	Cmd.m_pData = nullptr;
	AddCmd(Cmd);
	return CreateRenderTargetHandle(TextureHandle.Id());
}

void CGraphics_Threaded::DeleteRenderTarget(CRenderTargetHandle *pRenderTarget)
{
	if(!pRenderTarget->IsValid())
		return;
	CTextureHandle TextureHandle = RenderTargetHandleToTexture(*pRenderTarget);
	UnloadTexture(&TextureHandle);
	pRenderTarget->Invalidate();
}

IGraphics::CTextureHandle CGraphics_Threaded::GetRenderTargetTexture(CRenderTargetHandle RenderTarget)
{
	if(!RenderTarget.IsValid())
		return IGraphics::CTextureHandle();
	return RenderTargetHandleToTexture(RenderTarget);
}

void CGraphics_Threaded::CaptureScreenToRenderTarget(CRenderTargetHandle RenderTarget)
{
	CCommandBuffer::SCommand_RenderTarget_CaptureScreen Cmd;
	Cmd.m_Slot = RenderTarget.Id();
	AddCmd(Cmd);
}

void CGraphics_Threaded::BeginRenderTarget(CRenderTargetHandle RenderTarget)
{
	CCommandBuffer::SCommand_RenderTarget_Bind Cmd;
	Cmd.m_Slot = RenderTarget.Id();
	AddCmd(Cmd);
}
...
{
	CCommandBuffer::SCommand_RenderTarget_Unbind Cmd;
	AddCmd(Cmd);
}

void CGraphics_Threaded::RenderPostProcess(CRenderTargetHandle DstRenderTarget, EPostProcessShader Shader, CTextureHandle SrcTexture0, CTextureHandle SrcTexture1, const CPostProcessParams &Params)
{
	CCommandBuffer::SCommand_RenderPostProcess Cmd;
	Cmd.m_DstSlot = DstRenderTarget.Id();
	Cmd.m_SrcSlot0 = SrcTexture0.Id();
	Cmd.m_SrcSlot1 = SrcTexture1.Id();
	Cmd.m_Shader = Shader;
	Cmd.m_Params = Params;
	AddCmd(Cmd);
}
...
bool CGraphics_Threaded::UpdateTextureRaw(CTextureHandle TextureId, int x, int y, size_t Width, size_t Height, const CImageInfo &Image, bool IsMovedPointer)
{
	if(!TextureId.IsValid() || Width == 0 || Height == 0 || Image.m_Width != Width || Image.m_Height != Height)
		return false;
	...
	if(Image.m_Format == CImageInfo::FORMAT_RGBA)
	{
		...
		else
		{
			...
			mem_copy(pTmpData, Image.m_pData, MemSize);
			Cmd.m_pData = pTmpData;
		}
	}
	else
	{
		uint8_t *pTmpData;
		if(!ConvertToRgbaAlloc(pTmpData, Image))
			dbg_msg("graphics", "converted updated texture to RGBA");
		Cmd.m_pData = pTmpData;
	}
