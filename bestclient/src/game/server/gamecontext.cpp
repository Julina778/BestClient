...
{
	if(Context.GeBestClientVersion() >= VERSION_DDNET_MULTI_LASER)
	{
...
bool CGameContext::SnapPickup(const CSnapContext &Context, int SnapId, const vec2 &Pos, int Type, int SubType, int SwitchNumber, int Flags) const
{
	...
	else if(Context.GeBestClientVersion() >= VERSION_DDNET_ENTITY_NETOBJS)
	{
	...
	else
	{
		...
		pPickup->m_Type = Type;
		if(Context.GeBestClientVersion() < VERSION_DDNET_WEAPON_SHIELDS)
		{
...
void CGameContext::SendEmoticon(int ClientId, int Emoticon, int TargeBestClientId) const
{
	CNetMsg_Sv_Emoticon Msg;
	Msg.m_ClientId = ClientId;
	Msg.m_Emoticon = Emoticon;
	Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, TargeBestClientId);
}
...
void CGameContext::SendServerAlert(const char *pMessage)
{
	for(int ClientId = 0; ClientId < Server()->MaxClients(); ClientId++)
	{
		...
		if(m_apPlayers[ClientId]->GeBestClientVersion() >= VERSION_DDNET_IMPORTANT_ALERT)
		{
...
void CGameContext::SendModeratorAlert(const char *pMessage, int ToClientId)
{
	...
	if(m_apPlayers[ToClientId]->GeBestClientVersion() >= VERSION_DDNET_IMPORTANT_ALERT)
	{
...
void CGameContext::SendVoteStatus(int ClientId, int Total, int Yes, int No)
{
	...
	if(Total > VANILLA_MAX_CLIENTS && m_apPlayers[ClientId] && m_apPlayers[ClientId]->GeBestClientVersion() <= VERSION_DDRACE)
	{
...
void CGameContext::PreInpuBestClients(int ClientId, bool *pClients)
{
	...
	for(int Id = 0; Id < MAX_CLIENTS; Id++)
	{
		...
		if(Server()->GeBestClientVersion(Id) < VERSION_DDNET_PREINPUT)
			continue;
...
void CGameContext::OnClientEnter(int ClientId)
{
	...
	if(Server()->GeBestClientInfo(ClientId, &Info) && Info.m_GotDDNetVersion)
	{
...
{
	CPersistenBestClientData *pPersistent = (CPersistenBestClientData *)pData;
	if(!m_apPlayers[ClientId])
	{
		return false;
	}
...
{
	CPersistenBestClientData *pPersistentData = (CPersistenBestClientData *)pData;
	bool Spec = false;
	bool Afk = true;
	int LastWhisperTo = -1;
	if(pPersistentData)
...
{
	IServer::CClientInfo Info;
	dbg_assert(Server()->GeBestClientInfo(ClientId, &Info), "failed to get client info");
...
void CGameContext::OnIsDDNetLegacyNetMessage(const CNetMsg_Cl_IsDDNetLegacy *pMsg, int ClientId, CUnpacker *pUnpacker)
{
	...
	Server()->SeBestClientDDNetVersion(ClientId, DDNetVersion);
...
void CGameContext::OnChangeInfoNetMessage(const CNetMsg_Cl_ChangeInfo *pMsg, int ClientId)
{
	...
	if(Server()->WouldClientNameChange(ClientId, pMsg->m_pName) && !ProcessSpamProtection(ClientId))
	{
		...
		Server()->SeBestClientName(ClientId, pMsg->m_pName);
	...
	{
		SixupNeedsUpdate = true;
		Server()->SeBestClientClan(ClientId, pMsg->m_pClan);
	}

	if(Server()->ClientCountry(ClientId) != pMsg->m_Country)
		SixupNeedsUpdate = true;
	Server()->SeBestClientCountry(ClientId, pMsg->m_Country);
...
void CGameContext::UpdatePlayerMaps()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(!Server()->ClientIngame(i))
			continue;
		if(Server()->GeBestClientVersion(i) >= VERSION_DDNET_OLD)
			continue;
...
void CGameContext::SendRecord(int ClientId)
{
	...
	Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, ClientId);
	if(!Server()->IsSixup(ClientId) && GeBestClientVersion(ClientId) < VERSION_DDNET_MSG_LEGACY)
	{
...
{
	int ClientVersion = m_apPlayers[ClientId]->GeBestClientVersion();
...
void CGameContext::SendSaveCode(int Team, int TeamSize, int State, const char *pError, const char *pSaveRequester, const char *pServerName, const char *pGeneratedCode, const char *pCode)
{
	...
	for(int MemberId = 0; MemberId < MAX_CLIENTS; MemberId++)
	{
		...
		if(GeBestClientVersion(MemberId) >= VERSION_DDNET_SAVE_CODE)
		{
...
void CGameContext::WhisperId(int ClientId, int VictimId, const char *pMessage)
{
	...
	else if(GeBestClientVersion(ClientId) >= VERSION_DDNET_WHISPER)
	{
		CNetMsg_Sv_Chat Msg;
	...
	else if(GeBestClientVersion(VictimId) >= VERSION_DDNET_WHISPER)
	{
...
int CGameContext::GeBestClientVersion(int ClientId) const
{
	return Server()->GeBestClientVersion(ClientId);
}
...
CClientMask CGameContext::ClientsMaskExcludeClientVersionAndHigher(int Version) const
{
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		if(GeBestClientVersion(i) >= Version)
			continue;
		Mask.set(i);
	}
	return Mask;
...
bool CGameContext::RateLimitPlayerVote(int ClientId)
{
	...
	if(g_Config.m_SvDnsblVote && Server()->DistincBestClientCount() > 1)
	{
