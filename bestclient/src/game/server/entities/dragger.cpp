...
void CDragger::LookForPlayersToDrag()
{
	...
	for(int i = 0; i < NumPlayersInRange; i++)
	{
		...
		{
			const int &TargeBestClientId = pTarget->GetPlayer()->GetCid();
			// Solo players are dragged independently from the rest of the team
			if(pTarget->Teams()->m_Core.GetSolo(TargeBestClientId))
			{
				aIsTarget[TargeBestClientId] = true;
			}
			else
			{
				...
				if(aMinDistInTeam[TargetTeam] == 0 || aMinDistInTeam[TargetTeam] > Distance)
				{
					...
					aClosestTargetIdInTeam[TargetTeam] = TargeBestClientId;
				}
				aCanStillBeTeamTarget[TargeBestClientId] = true;
			}
		}
	}
...
bool CDragger::WillDraggerBeamUseDraggerId(int TargeBestClientId, int SnappingClientId)
{
	...
	CCharacter *pTargetChar = GameServer()->GetPlayerChar(TargeBestClientId);
	CCharacter *pSnapChar = GameServer()->GetPlayerChar(SnappingClientId);
	if(pTargetChar && pSnapChar && m_apDraggerBeam[TargeBestClientId] != nullptr)
	{
		...
		if(SnapTeam == TargetTeam && SnapTeam < MAX_CLIENTS)
		{
			...
			{
				return SnappingClientId == TargeBestClientId;
			}
			else
			{
				return m_aTargetIdInTeam[SnapTeam] == TargeBestClientId;
			}
		}
	}
	return false;
