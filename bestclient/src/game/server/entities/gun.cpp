...
void CGun::Fire()
{
	...
	for(int i = 0; i < NumPlayersInRange; i++)
	{
		...
		const bool &TargetIsSolo = pTarget->Teams()->m_Core.GetSolo(TargeBestClientId);
		if((TargetIsSolo &&
			   m_aLastFireSolo[TargeBestClientId] + Server()->TickSpeed() / g_Config.m_SvPlasmaPerSec > Server()->Tick()) ||
			(!TargetIsSolo &&
		...
		if(IsReachable && pTarget->IsAlive())
		{
			...
			if(TargetIsSolo)
			{
				...
				m_aLastFireSolo[TargeBestClientId] = Server()->Tick();
			}
			else
			{
				...
				if(aMinDistInTeam[TargetTeam] == 0 || aMinDistInTeam[TargetTeam] > Distance)
				{
					...
					aTargetIdInTeam[TargetTeam] = TargeBestClientId;
				}
			}
		}
	}
