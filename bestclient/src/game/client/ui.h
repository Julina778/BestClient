...
public:
	...
	using FLabelMatchCallback = bool (*)(void *pUser, const char *pText, const CUIRect *pRect);
...
private:
	...
	const void *m_pHoveredSoundItem = nullptr;
	const void *m_pBecomingHoveredSoundItem = nullptr;
	...
	FLabelMatchCallback m_pfnLabelMatchCallback = nullptr;
	void *m_pLabelMatchUser = nullptr;
...
public:
	...
	void SetLabelMatchCallback(FLabelMatchCallback pfnCallback, void *pUser);
	bool LabelMatchesFilter(const char *pText, const CUIRect *pRect) const;
	...
	int LastMouseButton(int Index) const { return (m_LastMouseButtons >> Index) & 1; } // BestClient
	...
	void SetHotScrollRegion(CScrollRegion *pId)
	{
		...
		else if(m_pBecomingHotScrollRegion == nullptr)
		{
	...
	// Bestclient
	void DoGradientLabel(const CUIRect *pRect, const char *pText, float Size, int Align, ColorRGBA Color1, ColorRGBA Color2, bool Horizontal) const;
