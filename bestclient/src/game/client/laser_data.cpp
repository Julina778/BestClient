...
CLaserData ExtractLaserInfo(int NetObjType, const void *pData, CGameWorld *pGameWorld, const CNetObj_EntityEx *pEntEx)
{
	...
	else
	{
		...
		Result.m_Owner = -1;
		// FNG servers commonly send vanilla laser objects without DDNet type metadata.
		// Treat them as rifle lasers so rifle-specific rendering stays stable after prediction.
		Result.m_Type = pGameWorld && pGameWorld->m_WorldConfig.m_IsFNG ? LASERTYPE_RIFLE : -1;
