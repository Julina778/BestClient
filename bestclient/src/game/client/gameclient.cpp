...
float EffectiveFastInputOffsetTicksLowDeltaMode(IClient *pClient, IServerBrowser *pServerBrowser)
{
	(void)pClient;
	(void)pServerBrowser;
...
float EffectiveFastInputOffsetTicks(IClient *pClient, IServerBrowser *pServerBrowser)
{
	...
	return EffectiveFastInputOffsetTicksLowDeltaMode(pClient, pServerBrowser);
}
...
void CGameClient::OnInit()
{
	...
	// propagate pointers
	m_UI.Init(Kernel());
	m_RenderTools.Init(Graphics(), TextRender(), this); // BestClient
...
void CGameClient::UpdatePositions()
{
	...
	if(Predict() && Client()->State() != IClient::STATE_DEMOPLAYBACK)
	{
		if(!AntiPingPlayers())
		{
...
void CGameClient::OnRender()
{
	...
	const auto IsWorldRenderComponent = [&](CComponent *pComponent) {
		return pComponent == &m_Background ||
			...
			pComponent == &m_Particles.m_RenderExtra ||
	...
	if(m_WorldEditor.ShouldUsePostProcess())
	{
		m_WorldEditor.BeginWorldRender();
		for(auto &pComponent : m_vpAll)
		{
			if(IsWorldRenderComponent(pComponent))
				pComponent->OnRender();
		}
		m_WorldEditor.EndWorldRender();
		for(auto &pComponent : m_vpAll)
		{
			if(!IsWorldRenderComponent(pComponent))
				pComponent->OnRender();
...
void CGameClient::FormaBestClientId(int ClientId, char (&aClientId)[16], EClientIdFormat Format) const
{
	...
	{
		const int HighesBestClientId = Format == EClientIdFormat::INDENT_AUTO ? m_Snap.m_HighesBestClientId : 64;
		const char *pFigureSpace = " ";
		char aNumber[8];
		str_format(aNumber, sizeof(aNumber), "%d", ClientId);
		aClientId[0] = '\0';
		if(ClientId < 100 && HighesBestClientId >= 100)
		{
			str_append(aClientId, pFigureSpace);
		}
		if(ClientId < 10 && HighesBestClientId >= 10)
		{
			str_append(aClientId, pFigureSpace);
		}
...
void CGameClient::ProcessEvents()
{
	...
	for(int Index = 0; Index < Num; Index++)
	{
		...
		else if(Item.m_Type == NETEVENTTYPE_HAMMERHIT)
		{
			...
			m_Effects.HammerHit(EventPos, Alpha, Volume);
			...
			for(int Conn = 0; Conn < NUM_DUMMIES; ++Conn)
			{
				...
				if(distance(Core.m_Pos, EventPos) <= ComboHammerHitRadius)
...
void CGameClient::OnNewSnapshot()
{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			else if(Item.m_Type == NETOBJTYPE_PLAYERINFO)
			{
				...
				if(pInfo->m_ClientId < MAX_CLIENTS && pInfo->m_ClientId == Item.m_Id)
				{
					...
					m_Snap.m_HighesBestClientId = maximum(m_Snap.m_HighesBestClientId, pInfo->m_ClientId);
	...
	else if(m_Snap.m_LocalClientId != m_PrevLocalId)
...
void CGameClient::OnPredict()
{
	...
	const bool EffectiveOthers = EffectiveFastInputOthers() || EffectiveLowDeltaOthers();
	if(FastInputTicks > 0 && !EffectiveOthers)
	...
	for(int Tick = Client()->GameTick(g_Config.m_ClDummy) + 1; Tick <= FinalTickSelf; Tick++)
	{
		...
		if(Tick > m_aLastNewPredictedTick[Dummy] && (Tick <= FinalTickRegular))
		{
			...
			if(Events & COREEVENT_HAMMER_HIT)
...
void CGameClient::UpdateRenderedCharacters()
{
	...
	const bool FastInputOthers = EffectiveFastInputOthers() || EffectiveLowDeltaOthers();
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		if(!m_Snap.m_aCharacters[i].m_Active)
			continue;
		...
		if(Predict() && (i == m_Snap.m_LocalClientId || IsPracticeParticipant || (AntiPingPlayers() && !IsOtherTeam(i))) && pChar)
		{
			...
			if(i == m_Snap.m_LocalClientId || IsPracticeParticipant)
			{
...
vec2 CGameClient::GetFreezePos(int ClientId)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		const bool ApplyFastInputLocal = ClientId == m_Snap.m_LocalClientId && FastInputTicks > 0;
		const bool ApplyFastInputOthers = ClientId != m_Snap.m_LocalClientId && FastInputOthers && FastInputTicks > 0;
