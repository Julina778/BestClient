...
void CUi::SetLabelMatchCallback(FLabelMatchCallback pfnCallback, void *pUser)
{
	m_pfnLabelMatchCallback = pfnCallback;
	m_pLabelMatchUser = pUser;
}

bool CUi::LabelMatchesFilter(const char *pText, const CUIRect *pRect) const
{
	return m_pfnLabelMatchCallback != nullptr && pText != nullptr && pText[0] != '\0' && pRect != nullptr &&
		m_pfnLabelMatchCallback(m_pLabelMatchUser, pText, pRect);
}
...
void CUi::OnCursorMove(float X, float Y)
{
	if(!CheckMouseLock())
	{
		m_UpdatedMousePos.x = std::clamp(m_UpdatedMousePos.x + X, 0.0f, maximum(0.0f, Graphics()->WindowWidth() - 1.0f));
		m_UpdatedMousePos.y = std::clamp(m_UpdatedMousePos.y + Y, 0.0f, maximum(0.0f, Graphics()->WindowHeight() - 1.0f));
	}
	...
	if(Enabled())
	{
		...
		if(m_TouchState.m_AnyPressed)
		{
			if(!CheckMouseLock())
			{
				...
				m_UpdatedMousePos.x = std::clamp(m_UpdatedMousePos.x, 0.0f, maximum(0.0f, WindowSize.x - 1.0f));
				m_UpdatedMousePos.y = std::clamp(m_UpdatedMousePos.y, 0.0f, maximum(0.0f, WindowSize.y - 1.0f));
			}
...
{
	const bool Highlight = LabelMatchesFilter(pText, pRect);
	if(Highlight)
	{
	...
	const ColorRGBA TextColorOld = TextRender()->GetTextColor();
	if(Highlight)
	...
	if(Highlight)
		TextRender()->TextColor(TextColorOld);
...
void CUi::DoLabelStreamed(CUIElement::SUIElementRect &RectEl, const CUIRect *pRect, const char *pText, float Size, int Align, const SLabelProperties &LabelProps, int StrLen, const CTextCursor *pReadCursor) const
{
	...
	if(RectEl.m_UITextContainer.Valid())
	{
		...
		const ColorRGBA TextColor = Highlight ? ColorRGBA(1.0f, 0.95f, 0.65f, 1.0f) : RectEl.m_TextColor;
		TextRender()->RenderTextContainer(RectEl.m_UITextContainer, TextColor, RectEl.m_TextOutlineColor, CursorPos.x, CursorPos.y);
	}
}
...
int CUi::DoDropDown(CUIRect *pRect, int CurSelection, const char **pStrs, int Num, SDropDownState &State)
{
	...
	const int SafeSelection = CurSelection >= 0 && CurSelection < Num ? CurSelection : -1;

	const auto LabelFunc = [SafeSelection, pStrs]() {
		return SafeSelection >= 0 ? pStrs[SafeSelection] : "";
	};
	...
	if(Num > 0 && DoButton_Menu(State.m_UiElement, &State.m_ButtonContainer, LabelFunc, pRect, Props))
	{
	...
	if(State.m_SelectionPopupContext.m_SelectionIndex >= 0)
	{
		...
		State.m_SelectionPopupContext.Reset();
		return NewSelection >= 0 && NewSelection < Num ? NewSelection : SafeSelection;
	}
...
// Gradient functions
void CUi::DoGradientLabel(const CUIRect *pRect, const char *pText, float Size, int Align, ColorRGBA Color1, ColorRGBA Color2, bool Horizontal) const
{
	if(!pText[0])
		return;

	CTextCursor Cursor;
	Cursor.SetPosition(vec2(pRect->x, pRect->y));
	Cursor.m_FontSize = Size;
	Cursor.m_LineWidth = pRect->w;

	// Calculate text bounds
	float TextWidth = TextRender()->TextWidth(Size, pText, -1);
	float TextHeight = Size;
	vec2 CursorPos = CalcAlignedCursorPos(pRect, vec2(TextWidth, TextHeight), Align);

	// Render text with gradient color based on position
	const int Length = str_length(pText);
	for(int i = 0; i < Length; i++)
	{
		char Char[2] = {pText[i], 0};
		float CharWidth = TextRender()->TextWidth(Size, Char, 1);
		
		// Calculate gradient factor for current character
	float GradientFactor;
	if(Horizontal)
		{
			GradientFactor = (CursorPos.x - pRect->x + (i / (float)Length) * TextWidth) / pRect->w;
		}
		else
		{
			GradientFactor = (CursorPos.y - pRect->y + (i / (float)Length) * TextHeight) / pRect->h;
		}
		GradientFactor = std::clamp(GradientFactor, 0.0f, 1.0f);

		// Interpolate color
		ColorRGBA CurrentColor = ColorRGBA(
			Color1.r + GradientFactor * (Color2.r - Color1.r),
			Color1.g + GradientFactor * (Color2.g - Color1.g),
			Color1.b + GradientFactor * (Color2.b - Color1.b),
			Color1.a + GradientFactor * (Color2.a - Color1.a)
		);

		TextRender()->TextColor(CurrentColor);
	TextRender()->TextEx(&Cursor, Char, 1);
