...
void CSpectator::OnRender()
{
	...
	if(!m_Active)
	{
		...
		if(m_WasActive)
		{
			...
			{
				if(m_SelectedSpectatorId == LOCK_CAM)
				{
					const int CurrentSpectatorId = Client()->State() == IClient::STATE_DEMOPLAYBACK ? GameClient()->m_DemoSpecId : GameClient()->m_Snap.m_SpecInfo.m_SpectatorId;
					if(GameClient()->m_LockCamClientId >= 0)
						GameClient()->m_LockCamClientId = -1;
					else if(CurrentSpectatorId >= 0 && CurrentSpectatorId < MAX_CLIENTS)
						GameClient()->m_LockCamClientId = CurrentSpectatorId;
				}
				else if(m_SelectedSpectatorId == MULTI_VIEW)
					GameClient()->m_MultiViewActivated = true;
				else if(m_SelectedSpectatorId == SPEC_FREEVIEW || m_SelectedSpectatorId == SPEC_FOLLOW)
					GameClient()->m_MultiViewActivated = false;

				if(m_SelectedSpectatorId != LOCK_CAM && !GameClient()->m_MultiViewActivated)
					Spectate(m_SelectedSpectatorId);

				if(GameClient()->m_MultiViewActivated && m_SelectedSpectatorId != MULTI_VIEW && m_SelectedSpectatorId != LOCK_CAM && GameClient()->m_Teams.Team(m_SelectedSpectatorId) != GameClient()->m_MultiViewTeam)
				{
	...
	const bool DemoFollowAvailable = Client()->State() == IClient::STATE_DEMOPLAYBACK && GameClient()->m_Snap.m_LocalClientId >= 0;
	const int TopButtonCount = DemoFollowAvailable ? 4 : 3;
	const float TopButtonStep = (ObjWidth * 2.0f) / TopButtonCount;
	const float TopButtonWidth = TopButtonStep - 40.0f;
	const float TopButtonStartX = Width / 2.0f - (ObjWidth - 20.0f);
	auto IsTopButtonHovered = [&](int Index) {
		const float ButtonX = -(ObjWidth - 20.0f) + TopButtonStep * Index;
		return m_SelectorMouse.x >= ButtonX && m_SelectorMouse.x <= ButtonX + TopButtonWidth &&
	...
	{
		Graphics()->DrawRect(TopButtonStartX, Height / 2.0f - 280.0f, TopButtonWidth, 60.0f, ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f), IGraphics::CORNER_ALL, 20.0f);
	}

	if(GameClient()->m_MultiViewActivated)
	{
		Graphics()->DrawRect(TopButtonStartX + TopButtonStep, Height / 2.0f - 280.0f, TopButtonWidth, 60.0f, ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f), IGraphics::CORNER_ALL, 20.0f);
	}

	if(GameClient()->m_LockCamClientId >= 0)
	{
		Graphics()->DrawRect(TopButtonStartX + TopButtonStep * 2.0f, Height / 2.0f - 280.0f, TopButtonWidth, 60.0f, ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f), IGraphics::CORNER_ALL, 20.0f);
	}

	if(DemoFollowAvailable && GameClient()->m_DemoSpecId == SPEC_FOLLOW)
	{
		Graphics()->DrawRect(TopButtonStartX + TopButtonStep * 3.0f, Height / 2.0f - 280.0f, TopButtonWidth, 60.0f, ColorRGBA(1.0f, 1.0f, 1.0f, 0.25f), IGraphics::CORNER_ALL, 20.0f);
	}

	bool FreeViewSelected = false;
	if(IsTopButtonHovered(0))
	{
	...
	TextRender()->Text(TopButtonStartX + 20.0f, Height / 2.0f - 280.f + (60.f - BigFontSize) / 2.f, BigFontSize, Localize("Free-View"), TopButtonWidth);

	if(IsTopButtonHovered(1))
	{
	...
	TextRender()->Text(TopButtonStartX + TopButtonStep + 20.0f, Height / 2.0f - 280.f + (60.f - BigFontSize) / 2.f, BigFontSize, Localize("Multi-View"), TopButtonWidth);

	bool LockCamSelected = false;
	if(IsTopButtonHovered(2))
	{
		m_SelectedSpectatorId = LOCK_CAM;
		LockCamSelected = true;
	}
	TextRender()->TextColor(1.0f, 1.0f, 1.0f, (LockCamSelected || GameClient()->m_LockCamClientId >= 0) ? 1.0f : 0.5f);
	TextRender()->Text(TopButtonStartX + TopButtonStep * 2.0f + 20.0f, Height / 2.0f - 280.f + (60.f - BigFontSize) / 2.f, BigFontSize, Localize(GameClient()->m_LockCamClientId >= 0 ? "Close Cam" : "Lock Cam"), TopButtonWidth);

	if(DemoFollowAvailable)
	{
		bool FollowSelected = false;
		if(IsTopButtonHovered(3))
		{
		...
		TextRender()->Text(TopButtonStartX + TopButtonStep * 3.0f + 20.0f, Height / 2.0f - 280.0f + (60.f - BigFontSize) / 2.f, BigFontSize, Localize("Follow"), TopButtonWidth);
	}
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
		...
		{
			char aClientId[16];
			GameClient()->FormaBestClientId(GameClient()->m_Snap.m_apInfoByDDTeamName[i]->m_ClientId, aClientId, EClientIdFormat::INDENT_AUTO);
		...
		if(GameClient()->m_LockCamClientId == GameClient()->m_Snap.m_apInfoByDDTeamName[i]->m_ClientId)
		{
			TextRender()->TextColor(1.0f, 0.85f, 0.1f, PlayerSelected ? 1.0f : 0.5f);
			TextRender()->Text(Width / 2.0f + x + 50.0f + 165.0f, Height / 2.0f + y + BoxMove + (LineHeight - FontSize) / 2.f, FontSize - 2, "L", 220.0f);
		}
