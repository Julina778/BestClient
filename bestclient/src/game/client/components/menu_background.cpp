...
void CMenuBackground::LoadThemeIcon(CTheme &Theme)
{
	...
	CImageInfo IconImage;
	if(Graphics()->LoadPng(IconImage, aIconPath, IStorage::TYPE_ALL))
	{
		if(IconImage.m_Width > 0 && IconImage.m_Height > 0)
			Theme.m_IconAspect = (float)IconImage.m_Width / (float)IconImage.m_Height;
		Theme.m_IconTexture = Graphics()->LoadTextureRawMove(IconImage, 0, aIconPath);
	}
	else
	{
	...
	if(!Theme.m_IconTexture.IsValid() || Theme.m_IconTexture.IsNullTexture())
