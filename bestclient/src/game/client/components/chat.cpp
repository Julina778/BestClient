...
#include <game/client/components/bestclient/colored_parts.h>
...
void CChat::RenderTranslateSettingsButton(const CUIRect &ButtonRect)
{
	...
	Ui()->DoLabel(&IconRect, FontIcons::FONT_ICON_COMMENT, IconSize, TEXTALIGN_MC);
...
std::string CChat::BuildPlainTextLine(const CLine &Line) const
{
	...
	{
		GameClient()->FormaBestClientId(Line.m_ClientId, aClientId, EClientIdFormat::INDENT_AUTO);
	}
...
bool CChat::OnInput(const IInput::CEvent &Event)
{
	...
	if((Event.m_Flags & IInput::FLAG_PRESS) && Event.m_Key == KEY_MOUSE_2)
	{
		...
		for(int i = m_BacklogCurLine; i < MAX_LINES; i++)
		{
			...
			if(!Line.m_Initialized)
				break;
			if(!Line.m_NameRectValid || Line.m_ClientId < 0)
				continue;

			const SRenderRect &Rect = Line.m_NameRect;
			if(MousePos.x >= Rect.m_X && MousePos.x <= Rect.m_X + Rect.m_W &&
				MousePos.y >= Rect.m_Y && MousePos.y <= Rect.m_Y + Rect.m_H)
			{
				GameClient()->m_Scoreboard.OpenPlayerPopup(Line.m_ClientId, UiMousePos.x, UiMousePos.y);
				return true;
			}
		}
	}
	...
	else if(Event.m_Flags & IInput::FLAG_PRESS && (Event.m_Key == KEY_RETURN || Event.m_Key == KEY_KP_ENTER))
	{
		...
		else if(GameClient()->m_BestClient.ChatDoSpecId(m_Input.GetString()))
...
void CChat::OnMessage(int MsgType, void *pRawMsg)
{
	...
	if(MsgType == NETMSGTYPE_SV_CHAT)
	{
		...
		auto &Re = GameClient()->m_BestClient.m_RegexChatIgnore;
