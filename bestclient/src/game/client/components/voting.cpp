...
{
	if((!g_Config.m_ClShowVotesAfterVoting && !GameClient()->m_Scoreboard.IsActive() && TakenChoice() && !ForcePreview) || (!IsVoting() && !ForcePreview))
		return;
	const int Seconds = SecondsLeft();
	if(Seconds < 0 && !ForcePreview)
	{
		OnReset();
		return;
	}
	...
	{
		const ColorRGBA BgColor = color_cast<ColorRGBA>(ColorHSLA(BackgroundColor, true));
		const int Corners = HudLayout::BackgroundCorners(IGraphics::CORNER_R, View.x, View.y, View.w, View.h, HudWidth, HudLayout::CANVAS_HEIGHT);
		View.Draw(BgColor, Corners, 3.0f * Scale);
	}
	...
	char aBuf[256];
	str_format(aBuf, sizeof(aBuf), Localize("%ds left"), ForcePreview ? maximum(Seconds, 12) : Seconds);
	...
	if(!ForcePreview && m_Closetime > m_Opentime)
	...
	Ui()->DoLabel(&LeftColumn, IsVoting() ? VoteDescription() : "Vote: kick", 6.0f * Scale, TEXTALIGN_ML, Props);

	View.HSplitTop(3.0f * Scale, nullptr, &View);
	View.HSplitTop(6.0f * Scale, &Row, &View);
	str_format(aBuf, sizeof(aBuf), "%s %s", Localize("Reason:"), IsVoting() ? VoteReason() : "Preview");
