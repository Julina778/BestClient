...
static void RotateMovingTilePoint(const vec2 &Center, vec2 &Point, float Rotation)
{
...
static bool MovingTileQuadName(const int *pInts, size_t NumInts, char *pStr, size_t StrSize)
{
	dbg_assert(NumInts > 0, "MovingTileQuadName: NumInts invalid");
	dbg_assert(StrSize >= NumInts * sizeof(int), "MovingTileQuadName: StrSize invalid");
...
void CMovingTiles::OnMapLoad()
{
	...
	for(int GroupIndex = 0; GroupIndex < GroupsNum; GroupIndex++)
	{
		...
		for(int LayerIndex = 0; LayerIndex < pGroup->m_NumLayers; LayerIndex++)
		{
			...
			MovingTileQuadName(pQuadLayer->m_aName, std::size(pQuadLayer->m_aName), aLayerName, std::size(aLayerName));

			for(size_t NameIndex = 0; NameIndex < std::size(gs_aMovingTileValidQuadNames); NameIndex++)
			{
				if(str_comp(gs_aMovingTileValidQuadNames[NameIndex], aLayerName) != 0)
					continue;

				EQType Type = static_cast<EQType>(NameIndex);
				...
				{
					CMovingTileQuadData QuadData;
					QuadData.m_pQuad = &pQuads[QuadIndex];
					QuadData.m_pGroup = pGroup;
					QuadData.m_pLayer = pQuadLayer;
					QuadData.m_Type = Type;
					for(int i = 0; i < 5; i++)
						QuadData.m_aPos[i] = vec2(fx2f(QuadData.m_pQuad->m_aPoints[i].x), fx2f(QuadData.m_pQuad->m_aPoints[i].y));
...
void CMovingTiles::OnRender()
{
	...
	auto RenderPass = [&](int RenderFlags) {
		...
		while(QuadStart < m_vQuads.size())
		{
			...
			while(QuadEnd < m_vQuads.size() && m_vQuads[QuadEnd].m_pGroup == pGroup && m_vQuads[QuadEnd].m_pLayer == pLayer)
				QuadEnd++;
			...
			{
				const CMovingTileQuadData &QuadData = m_vQuads[QuadIndex];
				...
				if(Rotation != 0.0f)
				{
					for(vec2 &Point : aPoints)
						RotateMovingTilePoint(QuadData.m_aPos[4], Point, Rotation);
				}
