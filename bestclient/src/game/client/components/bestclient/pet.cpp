...
bool CPet::ShouldProcess() const
{
	return g_Config.m_TcPetShow > 0 &&
		g_Config.m_TcPetAlpha > 0 &&
...
void CPet::OnRender()
{
	...
	{
		if(m_Alpha > 0.0f || length(m_Velocity) > 0.0f)
			ResetState();
		return;
	}
	...
	if(Player.m_Active && Player.m_Team != TEAM_SPECTATORS)
	{
		...
		const float DeltaLengthSq = dot(DeltaPosition, DeltaPosition);
		const float DeltaLength = sqrtf(DeltaLengthSq);
		if(DeltaLengthSq > 512.0f * 512.0f)
			m_Alpha = 0.01f;
	...
	else
	{
		...
		if(dot(DirMouse, DirMouse) > 1.0f)
			DirMouse = normalize(DirMouse);
		vec2 DirVel = m_Velocity;
		if(dot(DirVel, DirVel) > 1.0f)
