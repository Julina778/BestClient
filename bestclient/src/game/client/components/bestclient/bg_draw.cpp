...
#include <game/client/components/bestclient/bg_draw_file.h>
...
constexpr float AUTO_SAVE_RETRY_INTERVAL = 1.0f;
...
static void BgDrawBuildFilename(CGameClient &This, const char *pFilename, char *pOut, int OutSize)
{
	if(pFilename && pFilename[0] != '\0')
	{
		if(str_endswith_nocase(pFilename, ".csv"))
			str_format(pOut, OutSize, "bgdraw/%s", pFilename);
		else
			str_format(pOut, OutSize, "bgdraw/%s.csv", pFilename);
	}
	else
	{
		...
		str_format(pOut, OutSize, "bgdraw/%s_%s.csv", This.Client()->GetCurrentMap(), aSha256);
	}
}
...
static IOHANDLE BgDrawOpenFile(CGameClient &This, const char *pFilename, int Flags)
{
	...
	BgDrawBuildFilename(This, pFilename, aFilename, sizeof(aFilename));
...
class CBgDrawSaveJob : public IJob
{
	IStorage *m_pStorage;
	IConsole *m_pConsole;
	std::vector<CBgDrawItemData> m_vItems;
...
protected:
	void Run() override
	{
		...
		IOHANDLE Handle = m_pStorage->OpenFile(m_aFilename, IOFLAG_WRITE, IStorage::TYPE_SAVE);
		if(!Handle)
			return;

		for(const auto &Data : m_vItems)
		{
			if(State() == IJob::STATE_ABORTED)
				break;
			if(!BgDrawFile::Write(Handle, Data))
				break;
		}
		io_close(Handle);
	}
...
public:
	CBgDrawSaveJob(IStorage *pStorage, IConsole *pConsole, const char *pFilename, std::vector<CBgDrawItemData> &&vItems) :
		m_pStorage(pStorage),
		m_pConsole(pConsole),
		m_vItems(std::move(vItems))
	{
		(void)m_pConsole;
		Abortable(true);
		str_copy(m_aFilename, pFilename ? pFilename : "", sizeof(m_aFilename));
	}
};
...
bool CBgDraw::HasActiveInput() const
{
	return std::any_of(m_aInputData.begin(), m_aInputData.end(), [](InputMode Mode) {
		return Mode != InputMode::NONE;
	});
}

bool CBgDraw::IsEnabled() const
{
	return g_Config.m_TcBgDraw != 0;
}

void CBgDraw::ResetInputState()
{
...
bool CBgDraw::HasRenderWork() const
{
	if(!IsEnabled())
		return false;
	return !m_pvItems->empty() || HasActiveInput() || (g_Config.m_TcBgDrawAutoSaveLoad != 0 && m_Dirty);
}
...
void CBgDraw::OnRender()
{
	...
	if(m_NextAutoSave < 0)
	{
		...
		{
			// Avoid frame hitches: never write to disk from the render path.
			// Also defer while the user is actively drawing/erasing.
			const bool JobRunning = m_pAutoSaveJob && !m_pAutoSaveJob->Done();
			if(!JobRunning && m_Dirty && !m_pvItems->empty() && !HasActiveInput())
			{
				// Ensure folder exists (cheap) and snapshot data for background write.
				Storage()->CreateFolder("bgdraw", IStorage::TYPE_SAVE);
				char aFilename[IO_MAX_PATH_LENGTH];
				BgDrawBuildFilename(*GameClient(), nullptr, aFilename, sizeof(aFilename));

				std::vector<CBgDrawItemData> vSnapshot;
				vSnapshot.reserve(m_pvItems->size());
				for(const CBgDrawItem &Item : *m_pvItems)
					vSnapshot.emplace_back(Item.Data());

				m_pAutoSaveJob = std::make_shared<CBgDrawSaveJob>(Storage(), Console(), aFilename, std::move(vSnapshot));
				Engine()->AddJob(m_pAutoSaveJob);
				m_Dirty = false;
				m_NextAutoSave = AUTO_SAVE_INTERVAL;
			}
			else
			{
				// Retry soon if we deferred due to active input or an in-flight job.
				m_NextAutoSave = AUTO_SAVE_RETRY_INTERVAL;
			}
		}
		else
		{
	...
	for(CBgDrawItem &Item : *m_pvItems)
	{
		...
		const CBoundingBox &BBox = Item.BoundingBox();
		const bool InRangeX = BBox.m_Min.x < ScreenX1 && BBox.m_Max.x > ScreenX0;
		const bool InRangeY = BBox.m_Min.y < ScreenY1 && BBox.m_Max.y > ScreenY0;
...
void CBgDraw::Reset()
{
	if(m_pAutoSaveJob && !m_pAutoSaveJob->Done())
		m_pAutoSaveJob->Abort();
	m_pAutoSaveJob.reset();
	ResetInputState();
	m_pvItems->clear();
}
...
void CBgDraw::OnStateChange(int NewState, int OldState)
{
	...
	{
		if(IsEnabled() && g_Config.m_TcBgDrawAutoSaveLoad > 0)
			Save(nullptr, true);
	}
	Reset();
	m_NextAutoSave = AUTO_SAVE_INTERVAL;
