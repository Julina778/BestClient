...
void CCustomCommunities::ResetCustomCommunitiesState()
{
...
void CCustomCommunities::LoadCustomCommunitiesDDNetInfo()
{
	...
	json_value *pCommunitiesInfo = m_pCustomCommunitiesDDNetInfo;
	pServerBrowser->m_CustomCommunitiesFunction = [pCommunitiesInfo](std::vector<json_value *> &vCommunities) {
		const json_value &Communities = (*pCommunitiesInfo)["communities"];
...
bool CCustomCommunities::HasPendingInfoTask() const
{
	return m_pCustomCommunitiesDDNetInfoTask != nullptr;
}
...
void CCustomCommunities::OnShutdown()
{
	if(auto *pServerBrowser = dynamic_cast<CServerBrowser *>(ServerBrowser()))
		pServerBrowser->m_CustomCommunitiesFunction = nullptr;
	ResetCustomCommunitiesState();
}
