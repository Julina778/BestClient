...
constexpr float VIEW_MARGIN = 0.0f;
constexpr float FADE_IN = 0.25f;
constexpr float FADE_OUT = 0.25f;
constexpr float DEFAULT_DEPTH = 120.0f;
constexpr int PARTICLE_MAX_RENDERED = 120;
...
{
	const float Z = Pos.z;
	const float Scale = std::clamp(PROJ_DIST / (PROJ_DIST + Z), 0.5f, 1.6f);
...
const std::array<vec3, HEART_LOW_POINTS> &HeartLowVertices()
{
	...
	if(!s_Inited)
	{
		const auto &Hi = HeartVertices();
		for(int i = 0; i < HEART_LOW_POINTS; i++)
		{
			const int Src = (i * HEART_POINTS) / HEART_LOW_POINTS;
			s_aVerts[i] = Hi[std::clamp(Src, 0, HEART_POINTS - 1)];
		}
		s_Inited = true;
	}
	return s_aVerts;
...
void C3DParticles::OnStateChange(int OldState, int NewState)
{
	(void)OldState;
	(void)NewState;
	ResetParticles();
...
bool C3DParticles::ShouldRenderPrimary() const
{
...
void C3DParticles::OnRender()
{
	...
	CMapItemGroup *pGameGroup = Layers()->GameGroup();
	if(pGameGroup == nullptr)
	{
		ResetParticles();
		return;
	}
	...
	const float Depth = DEFAULT_DEPTH;

	const float FadeIn = FADE_IN;
	const float FadeOut = FADE_OUT;
	...
	RenderTools()->MapScreenToGroup(GameClient()->m_Camera.m_Center.x, GameClient()->m_Camera.m_Center.y, pGameGroup, GameClient()->m_Camera.m_Zoom);
	...
	const float ViewMargin = VIEW_MARGIN;
	const float SpawnMargin = 0.0f;
	...
	float SpawnMinX = std::max(ScreenX0 - SpawnMargin, MapMinX);
	float SpawnMaxX = std::min(ScreenX1 + SpawnMargin, MapMaxX);
	float SpawnMinY = std::max(ScreenY0 - SpawnMargin, MapMinY);
	float SpawnMaxY = std::min(ScreenY1 + SpawnMargin, MapMaxY);
	...
	int TargetCount = std::clamp(g_Config.m_Bc3dParticlesCount, 0, 200);
	if(GameClient()->m_BestClient.OptimizerDisableParticles())
		TargetCount = minimum(TargetCount, 32);
	else
		TargetCount = minimum(TargetCount, PARTICLE_MAX_RENDERED);
	if((int)m_vParticles.size() > TargetCount)
		m_vParticles.resize(TargetCount);

	const float PushRadius = 120.0f;
	const float PushStrength = 140.0f;
	...
	const bool EnableParticleCollisions = TargetCount <= 48;
	...
	for(auto &Part : m_vParticles)
	{
		...
		if((Part.m_Pos.x < ViewMinX || Part.m_Pos.x > ViewMaxX ||
			Part.m_Pos.y < ViewMinY || Part.m_Pos.y > ViewMaxY) && !Part.m_FadingOut)
		{
	...
	{
		const int Type = PickType(g_Config.m_Bc3dParticlesType);
		const float BaseSize = (float)g_Config.m_Bc3dParticlesSizeMax;
		const float Size = random_float(BaseSize * 0.75f, BaseSize * 1.15f);
		...
		{
			const ColorHSLA RandHsla(random_float(), 0.85f, 0.65f, BaseColor.a);
			BaseColor = color_cast<ColorRGBA>(RandHsla);
		}
		...
		vec3 Dir = vec3(random_float(-1.0f, 1.0f), random_float(-1.0f, 1.0f), 0.0f);
		...
		P.m_SpawnOffset = SpawnDir * (Size * random_float(0.35f, 0.85f));
		P.m_FadeOutOffset = FadeDir * (Size * random_float(0.55f, 1.1f));
	...
	RenderParticles(ViewMinX, ViewMaxX, ViewMinY, ViewMaxY, BaseAlpha, FadeIn, FadeOut, PrevX0, PrevY0, PrevX1, PrevY1);
}

void C3DParticles::RenderSecondary()
{
...
void C3DParticles::RenderParticles(float ViewMinX, float ViewMaxX, float ViewMinY, float ViewMaxY, float BaseAlpha, float FadeIn, float FadeOut, float PrevX0, float PrevY0, float PrevX1, float PrevY1)
{
	if(m_vParticles.empty())
	{
		Graphics()->MapScreen(PrevX0, PrevY0, PrevX1, PrevY1);
	...
	auto DrawHeartLines = [&](const SParticle &Part, const vec3 &RenderPos, float RenderSize, float FinalAlpha) {
		Graphics()->SetColor(ColorRGBA(Part.m_Color.r, Part.m_Color.g, Part.m_Color.b, Part.m_Color.a * FinalAlpha));
		...
		for(int L = 0; L < HEART_LAYERS - 1; L++)
		{
			...
			std::array<IGraphics::CLineItem, HEART_LOW_POINTS> aDiag;
			for(int i = 0; i < HEART_LOW_POINTS; i++)
			{
				const int Next = (i + 1) % HEART_LOW_POINTS;
				aVertical[i] = IGraphics::CLineItem(aProjected[L][i], aProjected[L + 1][i]);
				aDiag[i] = IGraphics::CLineItem(aProjected[L][i], aProjected[L + 1][Next]);
			}
			Graphics()->LinesDraw(aVertical.data(), aVertical.size());
			Graphics()->LinesDraw(aDiag.data(), aDiag.size());
		}
		...
		if(GlowEnabled && GlowAlpha > 0.0f && GlowOffset > 0.0f)
		{
			if(GetRenderParams(Part, GlowAlpha, GlowOffsetVec, RenderPos, RenderSize, FinalAlpha))
			{
				if(Part.m_Type == SHAPE_CUBE)
					...
					DrawHeartLines(Part, RenderPos, RenderSize, FinalAlpha);
			}
		}
