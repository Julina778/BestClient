...
decltype(CBindChat::BIND_DEFAULTS) CBindChat::BIND_DEFAULTS = {
	{TCLocalizable("Kaomoji"), {
					   {TCLocalizable("Shrug:"), {"!shrug", "chai BestClient/builtinscripts/sayemoticon.chai ¯\\_(ツ)_/¯D"}},
					   {TCLocalizable("Flip:"), {"!flip", "chai BestClient/builtinscripts/sayemoticon.chai (╯°□°)╯︵ ┻━┻D"}},
					   {TCLocalizable("Unflip:"), {"!unflip", "chai BestClient/builtinscripts/sayemoticon.chai ┬─┬ノ( º _ ºノ)D"}},
					   {TCLocalizable("Cute:"), {"!cute", "chai BestClient/builtinscripts/sayemoticon.chai ૮ ˶ᵔ ᵕ ᵔ˶ აD"}},
					   {TCLocalizable("Lenny:"), {"!lenny", "chai BestClient/builtinscripts/sayemoticon.chai ( ͡° ͜ʖ ͡°)D"}},
				   }},
...
void CBindChat::OnConsoleInit()
{
	...
	if(pConfigManager)
		pConfigManager->RegisterCallback(ConfigSaveCallback, this, ConfigDomain::BestClientCHATBINDS);
...
void CBindChat::ConfigSaveCallback(IConfigManager *pConfigManager, void *pUserData)
{
	...
	for(CBind &Bind : pThis->m_vBinds)
	{
		...
		str_append(aBuf, "\"");
		pConfigManager->WriteLine(aBuf, ConfigDomain::BestClientCHATBINDS);
	}
