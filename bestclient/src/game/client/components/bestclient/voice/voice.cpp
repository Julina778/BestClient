...
void CVoiceChat::RenderMenuPanelToggleBind(const CUIRect &View)
{
	...
	RenderBindRow(Localize("Voice panel bind"), "toggle_voice_panel", m_PanelBindReaderButton, m_PanelBindClearButton);
}
...
void CVoiceChat::RenderHudMuteStatusIndicator(float HudWidth, float HudHeight, bool ForcePreview)
{
	...
	const SVoiceStatusIcon aIcons[2] = {
		{FontIcons::FONT_ICON_MICROPHONE, ShowMicMuted},
		{FontIcons::FONT_ICON_HEADPHONES, ShowHeadphonesMuted},
	};
	...
	for(int i = 0; i < 2; ++i)
	{
		...
		if(Muted)
		{
			...
			const float CrossWidth = TextRender()->TextWidth(CrossSize, FontIcons::FONT_ICON_XMARK, -1, -1.0f);
			TextRender()->Text(CenterX - CrossWidth * 0.5f, CenterY - CrossSize * 0.5f, CrossSize, FontIcons::FONT_ICON_XMARK, -1.0f);
		}
	}
...
void CVoiceChat::RenderHudTalkingIndicator(float HudWidth, float HudHeight, bool ForcePreview)
{
	...
	if(ForcePreview && vEntries.empty())
	{
		...
		for(int Index = 0; Index < RenderCount; ++Index)
		{
			...
			const float MicGlyphWidth = TextRender()->TextWidth(IconSize, FontIcons::FONT_ICON_MICROPHONE, -1, -1.0f);
			TextRender()->Text(MicX + (IconWidth - MicGlyphWidth) * 0.5f, RowY + (RowHeight - IconSize) * 0.5f, IconSize, FontIcons::FONT_ICON_MICROPHONE, -1.0f);
			TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);
		}
...
void CVoiceChat::RenderServersSection(CUIRect View)
{
	...
	TextRender()->SetFontPreset(EFontPreset::ICON_FONT);
	Ui()->DoLabel(&CardIcon, FontIcons::FONT_ICON_NETWORK_WIRED, 14.0f, TEXTALIGN_MC);
	TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);
...
void CVoiceChat::RenderSettingsSection(CUIRect View)
{
	...
	RenderBindRow(Localize("PTT bind"), "+voicechat", m_PttBindReaderButton, m_PttBindClearButton);
	RenderBindRow(Localize("Mute microphone bind"), "toggle_voice_mic_mute", m_MicMuteBindReaderButton, m_MicMuteBindClearButton);
	RenderBindRow(Localize("Mute headphones bind"), "toggle_voice_headphones_mute", m_HeadphonesMuteBindReaderButton, m_HeadphonesMuteBindClearButton);
...
void CVoiceChat::RenderPanel(const CUIRect &Screen, bool ShowCloseButton)
{
	...
	TextRender()->SetFontPreset(EFontPreset::ICON_FONT);
	Ui()->DoLabel(&HeaderIcon, FontIcons::FONT_ICON_NETWORK_WIRED, 12.0f, TEXTALIGN_MC);
	TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);
	...
	if(ShowCloseButton)
	{
		const bool Close = GameClient()->m_Menus.DoButton_Menu(&m_ClosePanelButton, FontIcons::FONT_ICON_XMARK, 0, &Right);
		TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);
		if(Close)
			SetPanelActive(false);
	}
	...
	// Simplified panel: only settings (includes server list).
	{
		...
		TextRender()->SetFontPreset(EFontPreset::ICON_FONT);
		if(GameClient()->m_Menus.DoButton_Menu(&m_MicMuteButton, FontIcons::FONT_ICON_MICROPHONE, 0, &MicButton, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_ALL, 5.0f, 0.0f, VoiceMuteButtonColor(g_Config.m_BcVoiceChatMicMuted != 0)))
			ToggleVoiceMicMute();
		if(GameClient()->m_Menus.DoButton_Menu(&m_HeadphonesMuteButton, FontIcons::FONT_ICON_HEADPHONES, 0, &HeadphonesButton, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_ALL, 5.0f, 0.0f, VoiceMuteButtonColor(g_Config.m_BcVoiceChatHeadphonesMuted != 0)))
		...
		if(g_Config.m_BcVoiceChatMicMuted)
			Ui()->DoLabel(&MicButton, FontIcons::FONT_ICON_XMARK, 8.0f, TEXTALIGN_MC);
		if(g_Config.m_BcVoiceChatHeadphonesMuted)
			Ui()->DoLabel(&HeadphonesButton, FontIcons::FONT_ICON_XMARK, 8.0f, TEXTALIGN_MC);
		TextRender()->TextColor(TextRender()->DefaultTextColor());
		TextRender()->SetFontPreset(EFontPreset::DEFAULT_FONT);
	}
	return;
	...
	if(GameClient()->m_Menus.DoButton_Menu(&m_SectionRoomButton, FontIcons::FONT_ICON_NETWORK_WIRED, 0, &RailButton, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_ALL, 5.0f, 0.0f, VoiceIconButtonColor(m_ActiveSection == VOICE_SECTION_SERVERS)))
		m_ActiveSection = VOICE_SECTION_SERVERS;
	RailInner.HSplitTop(6.0f, nullptr, &RailInner);
	RailInner.HSplitTop(PANEL_SECTION_BUTTON_SIZE, &RailButton, &RailInner);
	if(GameClient()->m_Menus.DoButton_Menu(&m_SectionMembersButton, FontIcons::FONT_ICON_USERS, 0, &RailButton, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_ALL, 5.0f, 0.0f, VoiceIconButtonColor(m_ActiveSection == VOICE_SECTION_MEMBERS)))
		m_ActiveSection = VOICE_SECTION_MEMBERS;
	RailInner.HSplitTop(6.0f, nullptr, &RailInner);
	RailInner.HSplitTop(PANEL_SECTION_BUTTON_SIZE, &RailButton, &RailInner);
	if(GameClient()->m_Menus.DoButton_Menu(&m_SectionSettingsButton, FontIcons::FONT_ICON_GEAR, 0, &RailButton, BUTTONFLAG_LEFT, nullptr, IGraphics::CORNER_ALL, 5.0f, 0.0f, VoiceIconButtonColor(m_ActiveSection == VOICE_SECTION_SETTINGS)))
