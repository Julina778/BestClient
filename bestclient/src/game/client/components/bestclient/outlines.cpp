...
static SOutlineConfig GetOutlineConfig(int Type)
{
	switch(Type)
	{
	case OUTLINE_SOLID:
...
bool COutlines::ShouldRender() const
{
	if(!m_pMapData)
		return false;
...
void COutlines::OnRender()
{
	...
	const int MaxWidth = Graphics()->ScreenWidth() / MaxScale;
	const int MaxHeight = Graphics()->ScreenHeight() / MaxScale;
	if(EndX - StartX > MaxWidth || EndY - StartY > MaxHeight)
	{
		int EdgeX = (EndX - StartX) - MaxWidth;
		StartX += EdgeX / 2;
		EndX -= EdgeX / 2;
		int EdgeY = (EndY - StartY) - MaxHeight;
		StartY += EdgeY / 2;
		EndY -= EdgeY / 2;
	}
	...
	for(int y = StartY; y < EndY; y++)
	{
		for(int x = StartX; x < EndX; x++)
		{
			...
			if(Type == OUTLINE_NONE)
				continue;
			const SOutlineConfig Config = GetOutlineConfig(Type);
