...
void CMod::OnRender()
{
	...
	const bool HasModWeaponOverlay = Client()->State() == IClient::STATE_ONLINE && m_ModWeaponActiveId >= 0 && m_ModWeaponActiveTimeLeft > 0.0f;
	const int PlayerHitboxMode = g_Config.m_TcShowPlayerHitBoxes;
	if(!HasModWeaponOverlay && PlayerHitboxMode <= 0)
		return;
