...
void CCamera::UpdateCamera()
{
	...
	vec2 TargetPos = CanUseCameraInfo ? GameClient()->m_CursorInfo.Target() : GameClient()->m_Controls.GetCameraMousePos();
	...
	const bool CameraDriftDisabled = GameClient()->m_BestClient.IsComponentDisabled(CBestClient::COMPONENT_VISUALS_CAMERA_DRIFT);
	if(CameraDriftDisabled)
	{
		m_DriftTargetOffset = vec2(0.0f, 0.0f);
		m_DriftCurrentOffset = vec2(0.0f, 0.0f);
	}
	if(!CameraDriftDisabled && g_Config.m_BcCameraDrift && !IsBlockedCameraServer && !GameClient()->m_Snap.m_SpecInfo.m_Active)
	{
	...
	const bool DynamicFovDisabled = GameClient()->m_BestClient.IsComponentDisabled(CBestClient::COMPONENT_VISUALS_DYNAMIC_FOV);
	const bool DynamicFovActive = !DynamicFovDisabled && g_Config.m_BcDynamicFov && !IsBlockedCameraServer && !GameClient()->m_Snap.m_SpecInfo.m_Active;
	m_DynamicFovTarget = 1.0f;
	if(DynamicFovActive)
	{
