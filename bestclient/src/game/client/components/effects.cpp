...
void CEffects::MagicParticles(float radius, float size, float type, float alphadelay, float count) // STILL IN DEV!!! In future make MagicParticles -> ParticleTrails
{
	const int LocalID = GameClient()->m_Snap.m_LocalClientId;
	if(LocalID < 0 || LocalID >= MAX_CLIENTS)
		return;
	if(GameClient()->m_aClients[LocalID].m_Team == TEAM_SPECTATORS && !GameClient()->m_Snap.m_SpecInfo.m_Active)
		return;
	const int ParticleCount = maximum(0, (int)count);
	if(ParticleCount <= 0 || radius <= 0.0f || size <= 0.0f || alphadelay <= 0.0f)
		return;
	...
	for(int i = 0; i < ParticleCount; i++)
	{
		...
		p.m_LifeSpan = LifeSpan; // seems to have fixed it
		p.m_StartSize = size;
		p.m_EndSize = EndSize;
		p.m_Friction = Friction;
		p.m_Color = ColorRGBA(random_float(0, 1.0f), random_float(0, 1.0f), random_float(0, 1.0f), random_float(0.5f, 1.0f)); // todo: make a selectable color
		p.m_StartAlpha = p.m_Color.a;
		GameClient()->m_Particles.Add(CParticles::GROUP_PROJECTILE_TRAIL, &p, Client()->RenderFrameTime());
	}
}
