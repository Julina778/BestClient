...
void CTooltips::OnRender()
{
	if(m_ActiveTooltip.has_value())
	{
		...
		Rect.w = minimum(Rect.w, maximum(0.0f, pScreen->w - 2 * Margin));
		Rect.h = minimum(Rect.h, maximum(0.0f, pScreen->h - 2 * Margin));
		const float MaxRectX = maximum(Margin, pScreen->w - Rect.w - Margin);
		const float MaxRectY = maximum(Margin, pScreen->h - Rect.h - Margin);

		// Try the top side.
		if(Tooltip.m_Rect.y - Rect.h - Margin > pScreen->y)
		{
			Rect.x = std::clamp(Ui()->MouseX() - Rect.w / 2.0f, Margin, MaxRectX);
		...
		else if(Tooltip.m_Rect.x + Tooltip.m_Rect.w + Margin + Rect.w < pScreen->w)
		{
			...
			Rect.y = std::clamp(Ui()->MouseY() - Rect.h / 2.0f, Margin, MaxRectY);
		}
		// Try the left side.
