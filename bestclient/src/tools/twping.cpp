...
int main(int argc, const char **argv)
{
	...
	CNeBestClient NeBestClient;
	NeBestClient.Open(BindAddr);
	...
	NeBestClient.Send(&Packet);

	const int64_t StartTime = time_get();

	using namespace std::chrono_literals;
	const std::chrono::nanoseconds Timeout = std::chrono::nanoseconds(1s);
	net_socket_read_wait(NeBestClient.m_Socket, Timeout);

	NeBestClient.Update();

	SECURITY_TOKEN ResponseToken;
	while(NeBestClient.Recv(&Packet, &ResponseToken, false))
	{
